...
function OrderCard({ order, onDelivery, onShipment, onDocument }: { order: SupplierPortalOrder; onDelivery: () => void; onShipment: () => void; onDocument: (shipment: SupplierPortalShipment) => void }) {
  return <article className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
    ...
    <div className="mt-4 grid grid-cols-3 gap-3"><Value label="Cantidad" value={String(order.quantity)} /><Value label="Costo" value={money(order.unitCost)} /><Value label="Total" value={money(order.total)} /></div>
...
function OrderStatus({ order }: { order: SupplierPortalOrder }) { if (order.status === 'received') return <span className="inline-flex items-center gap-1 rounded-full bg-slate-100 px-2 py-1 text-xs font-bold text-slate-600"><PackageCheck className="h-3 w-3" />Recibida</span>; if (order.deliveryStatus === 'unavailable') return <span className="inline-flex items-center gap-1 rounded-full bg-rose-100 px-2 py-1 text-xs font-bold text-rose-700"><XCircle className="h-3 w-3" />No disponible</span>; if (order.shipment) return <span className="inline-flex items-center gap-1 rounded-full bg-blue-100 px-2 py-1 text-xs font-bold text-blue-700"><Send className="h-3 w-3" />Despachada</span>; if (order.deliveryStatus) return <span className="inline-flex items-center gap-1 rounded-full bg-emerald-100 px-2 py-1 text-xs font-bold text-emerald-700"><CheckCircle2 className="h-3 w-3" />Confirmada</span>; return <span className="inline-flex items-center gap-1 rounded-full bg-amber-100 px-2 py-1 text-xs font-bold text-amber-700"><Clock3 className="h-3 w-3" />Pendiente</span>; }
