...
import { 
  ShoppingCart, Barcode, Scale, AlertTriangle, Plus, Trash2, CheckCircle2, 
  ChevronRight, X, LayoutGrid, DollarSign, ArrowRightLeft, ShieldAlert,
  Printer, Trash, RefreshCw, Calculator, User, Award, ListFilter, Search
} from 'lucide-react';

type SupermarketCategory = 'almacen' | 'bebidas' | 'lacteos' | 'carniceria' | 'verduleria' | 'limpieza' | 'panaderia';
...
export const SUPERMARKET_PRODUCTS: SupermarketProduct[] = [
  { id: 's1', name: 'Leche Entera La Serenísima 1L', price: 1800, cost: 1200, barcode: '779123456001', isWeighed: false, unit: 'unit', category: 'lacteos', expirationDate: '2026-07-28', daysToExpire: 18, promo: 'none', stock: 120, minStock: 20, supplier: 'Mastellone SA' },
  { id: 's2', name: 'Queso Cremoso La Paulina (Trozado)', price: 8500, cost: 5800, barcode: '779123456002', isWeighed: true, unit: 'kg', category: 'lacteos', expirationDate: '2026-07-12', daysToExpire: 2, promo: '30off', stock: 8, minStock: 15, supplier: 'La Paulina SRL' },
  { id: 's3', name: 'Coca Cola Original 2.25L', price: 3400, cost: 2100, barcode: '779123456003', isWeighed: false, unit: 'unit', category: 'bebidas', expirationDate: '2026-09-15', daysToExpire: 65, promo: '2x1', stock: 250, minStock: 50, supplier: 'Femsa S.A.' },
  { id: 's4', name: 'Manzana Red Delicius', price: 2800, cost: 1600, barcode: '779123456004', isWeighed: true, unit: 'kg', category: 'verduleria', expirationDate: '2026-07-15', daysToExpire: 5, promo: 'none', stock: 12, minStock: 30, supplier: 'Central Frutera' },
  { id: 's5', name: 'Detergente Ala Platos 500ml', price: 2100, cost: 1400, barcode: '779123456005', isWeighed: false, unit: 'unit', category: 'limpieza', expirationDate: '2027-02-10', daysToExpire: 200, promo: 'none', stock: 45, minStock: 10, supplier: 'Unilever Arg' },
  { id: 's6', name: 'Mignon de Panadería (Bolsa)', price: 2500, cost: 1500, barcode: '779123456006', isWeighed: true, unit: 'kg', category: 'panaderia', expirationDate: '2026-07-11', daysToExpire: 1, promo: 'none', stock: 5, minStock: 25, supplier: 'PanSur Distribuidora' }
];
...
export default function SupermarketConsole({ products = SUPERMARKET_PRODUCTS, onSaleCommitted }: SupermarketConsoleProps) {
  ...
  const handleAddProduct = (prod: SupermarketProduct, explicitQty: number = 1) => {
    setCart(prev => {
      const existing = prev.find(item => item.product.id === prod.id);
      let newQty = explicitQty;
      let newWeight = simulatedWeight * explicitQty;

      if (existing) {
        newQty = existing.qty + explicitQty;
        newWeight = (existing.weight || 0) + (simulatedWeight * explicitQty);
      }
      ...
      let subtotal = prod.isWeighed ? newWeight * unitPrice : newQty * unitPrice;
      ...
      if (existing) {
        return prev.map(item => item.product.id === prod.id
          ? { ...item, qty: newQty, weight: prod.isWeighed ? newWeight : undefined, subtotal }
          : item
        );
      }

      return [...prev, { product: prod, qty: newQty, weight: prod.isWeighed ? newWeight : undefined, subtotal }];
    });
  ...
  const handleBarcodeSubmit = (e: React.FormEvent) => {
    ...
    const prod = localProducts.find(p => p.barcode === barcodeInput || p.id === barcodeInput);
    if (prod) {
      handleAddProduct(prod, posMultiplier);
      setBarcodeInput('');
    } else {
      setFeedback({ type: 'error', message: 'Código de barras no reconocido.' });
    }
  };
  ...
  const filteredProducts = useMemo(() => localProducts.filter(p => p.active !== false).filter(p => {
    ...
    <div className="w-full max-w-7xl mx-auto px-4 py-6 text-slate-100 flex flex-col gap-6 font-sans">
      ...
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center bg-slate-900/60 p-5 rounded-2xl border border-slate-800 gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-black text-white">Terminal POS de Alta Velocidad (Caja Rápida)</h1>
            <p className="text-slate-400 text-xs mt-0.5">Control de arqueos de caja, anulaciones, devoluciones de mercadería y promociones.</p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
        ...
        <div className="bg-slate-900/50 p-4 rounded-xl border border-slate-850">
          <span className="text-[10px] text-slate-500 font-bold uppercase block">Margen Estimado</span>
          <span className="text-xl font-black text-emerald-450 mt-1 block">
            ${(salesHistory.reduce((acc, curr) => acc + curr.margin, 0)).toLocaleString()}
          </span>
        </div>
        <div className="bg-slate-900/50 p-4 rounded-xl border border-slate-850">
        ...
        <div className="bg-slate-900/50 p-4 rounded-xl border border-slate-850">
          <span className="text-[10px] text-slate-500 font-bold uppercase block">Anulaciones</span>
          <span className="text-xl font-black text-amber-500 mt-1 block">{voidLogs.length}</span>
        </div>
      </div>
      ...
      {localProducts.some(p => p.daysToExpire <= 5) && (
        <div className="p-4 bg-amber-550/10 border border-amber-500/25 rounded-2xl flex items-center justify-between">
          <div className="flex items-center gap-2.5 text-xs text-amber-300 font-semibold">
            ...
            <span>Alerta de Góndola: Existen productos por vencer en menos de 5 días.</span>
          </div>
        </div>
      )}
      ...
      <div className="bg-slate-900/40 p-5 rounded-2xl border border-slate-850 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3">
        <div className="space-y-1 col-span-1 sm:col-span-2">
          <label className="text-[9px] text-slate-500 font-bold uppercase block">Buscar Producto por EAN / Nombre</label>
        ...
        <div className="space-y-1">
          <label className="text-[9px] text-slate-500 font-bold uppercase block">Categoría Góndola</label>
          <select
        ...
        <div className="space-y-1">
          <label className="text-[9px] text-slate-500 font-bold uppercase block">Proveedor Distribuidor</label>
          <select
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 flex flex-col gap-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            {/* Escáner de Entrada */}
            <form onSubmit={handleBarcodeSubmit} className="bg-slate-900/40 p-5 rounded-3xl border border-slate-850 space-y-3">
              <label className="text-[10px] text-slate-500 font-bold uppercase block">Simulador Pistola / EAN Manual</label>
              <div className="flex gap-2">
                ...
                />
                <button type="submit" className="px-4 bg-cyan-600 hover:bg-cyan-500 text-slate-950 font-bold rounded-xl text-xs">
                  Escanear
                </button>
              </div>
                  ...
                  {[1, 2, 5, 10].map(n => (
                    <button
                      ...
                      className={`px-3 py-1 rounded-lg border text-[10px] font-bold ${
                        posMultiplier === n ? 'bg-cyan-600 text-slate-950 border-cyan-500' : 'bg-slate-950 border-slate-850 text-slate-400'
                      }`}
                    >
                      x{n}
                    </button>
            ...
            <div className="bg-slate-900/40 p-5 rounded-3xl border border-slate-850 space-y-3">
              <label className="text-[10px] text-slate-500 font-bold uppercase block">Balanza de Pesaje ($ / KG)</label>
              <div className="flex items-center justify-between">
                ...
                <div className="flex gap-1.5">
                  <button onClick={() => setSimulatedWeight(w => Math.max(0.1, w - 0.25))} className="p-2 bg-slate-950 rounded-lg text-[10px] border border-slate-850 hover:bg-slate-900">-250g</button>
                  <button onClick={() => setSimulatedWeight(w => w + 0.25)} className="p-2 bg-slate-950 rounded-lg text-[10px] border border-slate-850 hover:bg-slate-900">+250g</button>
                </div>
              </div>
            </div>
          ...
          <div className="bg-slate-900/30 p-5 rounded-3xl border border-slate-850 space-y-3">
            <span className="text-xs font-bold text-slate-400 uppercase tracking-wider block">Góndola Comercial</span>
                  ...
                  >
                    <div>
                      ...
                      <div className="flex gap-1 items-center mt-1">
                        ...
                        {isCritical && <span className="text-[8px] text-rose-400 font-bold uppercase">Reponer</span>}
                        {isExpiring && <span className="text-[8px] text-amber-550 font-bold uppercase">Vence Pronto</span>}
                      </div>
                    </div>
                    <div className="text-right shrink-0">
                      <span className="text-cyan-400 font-black">${p.price.toLocaleString()}</span>
                      <span className="text-[9px] text-slate-550 block">Stock: {p.stock}</span>
                    </div>
                  </div>
                );
              })}
          ...
          <div className="bg-slate-900/40 p-5 rounded-3xl border border-slate-850 space-y-3">
            <span className="text-xs font-bold text-slate-450 uppercase tracking-wider block">Devolución / Rechazo de Producto</span>
            <form onSubmit={handleRegisterReturn} className="grid grid-cols-1 md:grid-cols-3 gap-3">
              <input
                type="text"
                required
              ...
              >
                <option value="Vencimiento / Mal Estado">Mal Estado / Vencido</option>
                <option value="Cambio de Producto">Cambio de Producto</option>
              </select>
              <button type="submit" className="py-2 bg-slate-955 border border-slate-800 text-slate-300 font-bold rounded-xl text-xs">
        ...
        <div className="bg-slate-900/40 p-6 rounded-3xl border border-slate-850 flex flex-col justify-between min-h-[520px]">
          <div>
            <div className="flex justify-between items-center pb-4 border-b border-slate-850">
              <h3 className="font-extrabold text-xs text-white uppercase tracking-wider">Detalle de Ticket</h3>
              <button onClick={() => setCart([])} className="text-[9px] text-rose-450 hover:underline">Vaciar Todo</button>
            </div>
            ...
            <div className="mt-4 space-y-2.5 max-h-64 overflow-y-auto pr-1">
              {cart.length === 0 ? (
                <div className="text-center py-24 text-slate-650 text-xs italic">
                  Escanear código de barras o presionar productos de la góndola comercial.
                </div>
              ) : (
                cart.map(item => (
                  <div key={item.product.id} className="flex justify-between items-center text-xs bg-slate-950/40 p-2.5 rounded-xl border border-slate-850">
                    <div className="flex-1 pr-2">
                      <span className="text-slate-200 block font-semibold">{item.product.name}</span>
                      {item.product.isWeighed ? (
                        <span className="text-[9px] text-slate-500">{item.weight?.toFixed(3)} kg x ${item.product.price}</span>
                      ) : (
                        <span className="text-[9px] text-slate-500">{item.qty} uds x ${item.product.price}</span>
                      )}
                    </div>
                    <div className="flex items-center gap-3 shrink-0">
                      <span className="font-bold text-slate-250">${item.subtotal.toLocaleString()}</span>
                      <button onClick={() => handleCancelItem(item.product.id)} className="p-1 hover:bg-rose-950 hover:text-rose-400 text-slate-650 rounded-lg">
          ...
          <div className="pt-4 border-t border-slate-850 space-y-4">
            <div className="flex justify-between items-center text-xs font-bold">
              ...
              <span className="text-xl font-black text-cyan-400">${cartTotal.toLocaleString()}</span>
            </div>
      ...
      <div className="bg-slate-900/40 p-5 rounded-3xl border border-slate-850 space-y-4">
        ...
        <div className="space-y-2 max-h-32 overflow-y-auto pr-1">
          {voidLogs.length === 0 ? (
            <span className="text-xs text-slate-600 italic block">No se registraron anulaciones ni devoluciones en este turno de caja.</span>
          ) : (
            voidLogs.map(log => (
              <div key={log.id} className="p-3 bg-slate-950 rounded-xl border border-slate-900 flex justify-between items-center text-xs">
                <div>
                  <span className="text-[11px] font-black text-slate-300 block">{log.productName}</span>
                  <span className="text-[10px] text-slate-500">Causa: {log.reason}</span>
                </div>
                <span className="text-[10px] text-rose-500 font-bold shrink-0">-${Math.abs(log.value).toLocaleString()}</span>
              </div>
            ))
          )}
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-slate-900/40 p-6 rounded-3xl border border-slate-850 space-y-4">
          ...
          <div className="space-y-3">
            {localProducts.map(p => {
              const statusColor = p.daysToExpire <= 2 
              ...
              return (
                <div key={p.id} className="p-3 bg-slate-950 rounded-2xl border border-slate-850 flex justify-between items-center text-xs font-mono">
                  <div>
                    ...
                    {p.daysToExpire <= 5 && !p.name.includes('(Liquidado)') && (
                      <button
                        onClick={() => {
                          ...
                          setFeedback({ type: 'success', message: `Aplicado descuento del 50% por liquidación a ${p.name}.` });
                        }}
        ...
        <div className="bg-slate-900/40 p-6 rounded-3xl border border-slate-850 space-y-4">
          ...
          <div className="space-y-3">
            {localProducts.filter(p => p.stock < p.minStock).length === 0 ? (
              ...
              localProducts.filter(p => p.stock < p.minStock).map(p => (
                <div key={p.id} className="p-3 bg-slate-950 rounded-2xl border border-slate-850 flex justify-between items-center text-xs font-mono">
                  <div>
                    <span className="font-extrabold text-slate-200 block">{p.name}</span>
                    <span className="text-[10px] text-slate-500">
                      Stock: <strong className="text-rose-500">{p.stock}</strong> / Min: {p.minStock} | Costo Unit: ${p.cost}
                    </span>
                  </div>
                  <button
                    onClick={() => {
        ...
        <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white border border-slate-200 rounded-3xl p-6 max-w-md w-full space-y-6 shadow-2xl text-slate-900">
            <div className="flex justify-between items-start border-b border-slate-100 pb-3">
              <div>
                <h4 className="font-extrabold text-sm text-slate-900">ARQUEO DE CAJA DIARIO</h4>
                <span className="text-[10px] text-slate-500 block mt-0.5">Responsable: {cashierName}</span>
              </div>
              <button onClick={() => setIsArqueoOpen(false)} className="text-slate-400 hover:text-slate-650">
                <X className="h-5 w-5" />
              </button>
            </div>
            ...
            <div className="space-y-3 text-xs">
              <div className="flex justify-between">
                ...
                <span className="font-bold text-slate-800">${cashBalance.toLocaleString()}</span>
              </div>

              <div className="space-y-1">
                <label className="text-[10px] text-slate-500 font-bold uppercase block">Efectivo Contado (Físico)</label>
                <input
                  type="number"
                  value={countedCash}
              ...
              <div className="flex justify-between border-t border-slate-100 pt-3 text-xs">
                ...
                <span className={`font-black ${
                  countedCash - cashBalance === 0 ? 'text-emerald-600' : 'text-rose-600'
                }`}>
      ...
      {printedTicketText && (
        <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white border border-slate-200 rounded-3xl p-6 max-w-sm w-full space-y-6 shadow-2xl text-slate-900">
            ...
            <button
              onClick={() => {
                setFeedback({ type: 'success', message: 'Comprobante térmico enviado a impresión.' });
