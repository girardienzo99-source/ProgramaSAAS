...
const INITIAL_DEVICES: DeviceItem[] = [
  {
    ...
    diagnosticNotes:
      "Pantalla con parpadeos verdes tras recalentamiento. Requiere cambio de módulo.",
    historicalTimeline: [
      {
        statusName: "Venta Registrada",
        date: "2025-10-12",
        comment: "Garantía oficial activa.",
      },
      {
        statusName: "Ingreso RMA",
        date: "2026-07-01",
        comment: "Presenta falla en panel OLED.",
      },
    ],
  },
  {
  ...
  },
  {
    ...
    diagnosticNotes:
      "Batería inflada. Reemplazada por servicio técnico oficial.",
    historicalTimeline: [
      {
        statusName: "Venta Registrada",
        date: "2025-01-20",
        comment: "Garantía extendida contratada.",
      },
      {
        statusName: "Ingreso RMA",
        date: "2026-06-15",
        comment: "Falla en carga de batería.",
      },
      {
        statusName: "Reparado",
        date: "2026-06-20",
        comment: "Módulo de batería reemplazado.",
      },
    ],
  },
];
...
export default function ElectronicsConsole() {
  ...
  const [selectedDevice, setSelectedDevice] = useState<DeviceItem | null>(
    INITIAL_DEVICES[0],
  );
  ...
  // FASE 3: Diagnóstico de Componentes y Garantías
  const [motherboardOk, setMotherboardOk] = useState(true);
  const [batteryOk, setBatteryOk] = useState(true);
  const [displayOk, setDisplayOk] = useState(false);
  const [selectedPart, setSelectedPart] = useState("Módulo de Pantalla OLED");
  const [warrantyYearsSim, setWarrantyYearsSim] = useState(1);

  // Solicitar repuesto al pañol
  const handleRequestPart = () => {
    if (!selectedDevice || !selectedPart) return;

    const newTimelineEntry: TimelineEntry = {
      statusName: "Repuesto Solicitado",
      date: todayISO(),
      comment: `Se solicitó el componente: ${selectedPart} para reparación.`,
    };
    ...
    setDevices((prev) =>
      prev.map((d) => (d.id === selectedDevice.id ? updatedDevice : d)),
    );
    setSelectedDevice(updatedDevice);
    setFeedback({
      tone: "warning",
      message: `Solicitud preparada para ${selectedPart}. Falta confirmarla contra el inventario persistente del depósito.`,
    });
  };

  // Contratar garantía extendida
  const handleApplyWarrantyExtension = () => {
    if (!selectedDevice) return;
    const premium =
      warrantyYearsSim === 1 ? 18500 : warrantyYearsSim === 2 ? 32000 : 45000;
    setFeedback({
      tone: "warning",
      message: `Cotización calculada: $${premium.toLocaleString()} por ${warrantyYearsSim} año(s). No se contrató ni cobró una cobertura.`,
    });
  };
  ...
  const handleCreateRMA = (e: React.FormEvent) => {
    ...
    if (!client || !model || !serialNumber) {
      setFeedback({
        tone: "error",
        message:
          "Completá cliente, modelo y número de serie para registrar el ingreso.",
      });
      return;
    }
      ...
      historicalTimeline: [
        {
          statusName: "Ingreso RMA",
          date: todayISO(),
          comment: "Equipo ingresado al laboratorio.",
        },
      ],
    };
    ...
    setFeedback({
      tone: "success",
      message: `Ingreso RMA generado para ${newDevice.deviceModel}.`,
    });
  ...
  const handleUpdateRMAStatus = (status: Exclude<RmaStatus, "none">) => {
    ...
    const updated: DeviceItem = {
      ...
      historicalTimeline: [
        ...selectedDevice.historicalTimeline,
        {
          statusName: `Estado: ${RMA_LABELS[status]}`,
          date: todayISO(),
          comment: "Cambio de etapa en laboratorio.",
        },
      ],
    };
    ...
    setFeedback({
      tone: "success",
      message: `RMA actualizado a ${RMA_LABELS[status]}.`,
    });
  };
  ...
  const handleSaveDiagnostic = () => {
    ...
    setFeedback({
      tone: "success",
      message: "Diagnóstico y peritaje del laboratorio guardados.",
    });
  };
  ...
  const metrics = useMemo(() => {
    const activeWarranties = devices.filter(
      (d) => d.warrantyStatus === "active" || d.warrantyStatus === "extended",
    ).length;
    const returnedDevices = devices.filter(
      (d) => d.rmaStatus === "replaced" || d.rmaStatus === "rejected",
    ).length;
    const returnRate =
      devices.length > 0
        ? Math.round((returnedDevices / devices.length) * 100)
        : 0;
    ...
    return {
      ...
      activeWarranties,
      delayedRma: devices.filter((d) => d.rmaStatus === "sent_to_supplier")
        .length,
      returnRate,
    };
  }, [devices]);
  ...
  const filteredDevices = useMemo(
    () =>
      devices.filter((d) => {
        ...
        const matchWarranty =
          warrantyFilter === "all" || d.warrantyStatus === warrantyFilter;
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-black text-white">
              Laboratorio de Soporte Técnico & RMA
            </h1>
            <p className="text-slate-400 text-xs mt-0.5">
              Control de números de serie/IMEI, diagnóstico técnico y
              trazabilidad de garantías oficiales.
            </p>
          </div>
        </div>
      ...
      {devices.some((d) => d.rmaStatus === "sent_to_supplier") && (
        <div className="p-4 bg-rose-550/10 border border-rose-500/25 rounded-2xl flex items-center justify-between">
          <div className="flex items-center gap-2.5 text-xs text-rose-350 font-semibold">
            ...
            <span>
              Alerta de RMA: Hay dispositivos demorados en el taller oficial del
              fabricante (ASUS/Apple).
            </span>
          </div>
        </div>
      )}
      ...
      <div className="bg-slate-900/40 p-5 rounded-2xl border border-slate-850 grid grid-cols-1 sm:grid-cols-3 gap-3">
        ...
        <div className="space-y-1">
          ...
          <select
            ...
            onChange={(e) => {
              if (isWarrantyFilter(e.target.value))
        ...
        <div className="lg:col-span-2 bg-slate-900/40 p-6 rounded-3xl border border-slate-850 space-y-6">
          {selectedDevice ? (
            <>
              <div className="flex justify-between items-start pb-4 border-b border-slate-850">
                <div>
                  ...
                  <span className="text-[10px] text-slate-500 block mt-0.5">
                    Serie: {selectedDevice.serialNumber} | Compra:{" "}
                    {selectedDevice.purchaseDate} | Vto Garantía:{" "}
                    {selectedDevice.warrantyExpiration}
                  </span>
                </div>
                ...
                <div className="flex gap-2">
                  <a
                    href="#rma-history"
                    className="px-3 py-1.5 bg-indigo-950/60 border border-indigo-500/30 text-indigo-300 hover:text-white font-bold rounded-xl text-[10px] flex items-center gap-1"
                  >
                ...
                <div className="flex gap-3 pt-1 text-slate-350">
                  {selectedDevice.accessoriesReceived.length === 0 ? (
                    <span className="text-slate-600 italic">
                      Ningún accesorio entregado.
                    </span>
                  ) : (
                    selectedDevice.accessoriesReceived.map((a, i) => (
                      <span
                        key={i}
                        className="px-2 py-0.5 bg-slate-900 rounded border border-slate-800 text-[10px]"
                      >
                        ✓ {a}
                      </span>
                    ))
              ...
              {/* FASE 3: Checklist Diagnóstico de Hardware y Solicitud de Repuesto */}
              <div className="p-4 bg-slate-950 border border-slate-850 rounded-2xl text-xs space-y-4">
                <div className="flex justify-between items-center pb-2 border-b border-slate-900">
                  <span className="text-[10px] text-slate-550 font-bold uppercase block text-left">
                    Control de Diagnóstico e Inspección Física
                  </span>
                  <span className="text-[9px] text-slate-500">
                    Almacén Central SASS
                  </span>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="space-y-2 text-left">
                    <label className="text-[9px] text-slate-500 font-bold uppercase block">
                      Prueba de Componentes Integrados
                    </label>
                    <div className="space-y-1.5">
                      <label className="flex items-center gap-2 cursor-pointer select-none text-slate-350">
                        <input
                          type="checkbox"
                          checked={motherboardOk}
                          onChange={(e) => setMotherboardOk(e.target.checked)}
                          className="cursor-pointer text-cyan-550"
                        />
                        <span>
                          Placa Madre / SoC (Test:{" "}
                          {motherboardOk ? "OK" : "Falla"})
                        </span>
                      </label>
                      <label className="flex items-center gap-2 cursor-pointer select-none text-slate-350">
                        <input
                        ...
                        />
                        <span>
                          Módulo de Batería (Salud: {batteryOk ? "OK" : "Falla"}
                          )
                        </span>
                      </label>
                      <label className="flex items-center gap-2 cursor-pointer select-none text-slate-350">
                        <input
                          type="checkbox"
                          checked={displayOk}
                          onChange={(e) => setDisplayOk(e.target.checked)}
                          className="cursor-pointer text-cyan-550"
                        />
                        <span>
                          Módulo Pantalla / Flex (Touch:{" "}
                          {displayOk ? "OK" : "Falla"})
                        </span>
                      </label>
                    </div>
                  ...
                  <div className="space-y-2 text-left">
                    <label className="text-[9px] text-slate-500 font-bold uppercase block">
                      Solicitar Repuesto de Reposición
                    </label>
                    <div className="flex gap-2">
                      <select
                        value={selectedPart}
                        onChange={(e) => setSelectedPart(e.target.value)}
                        className="flex-1 bg-slate-900 border border-slate-800 rounded-xl px-2 py-1.5 text-[10px] text-white focus:outline-none"
                      >
                        <option value="Módulo de Pantalla OLED">
                          Módulo de Pantalla OLED
                        </option>
                        <option value="Batería de Reemplazo Li-ion">
                          Batería de Reemplazo Li-ion
                        </option>
                        <option value="Pin de Carga USB-C">
                          Pin de Carga USB-C
                        </option>
                        <option value="Chip de Audio Integrado">
                          Chip de Audio Integrado
                        </option>
                      </select>
                      <button
                        onClick={handleRequestPart}
                        className="px-3 py-1.5 bg-cyan-600 hover:bg-cyan-500 text-slate-950 font-bold rounded-xl text-[10px] uppercase cursor-pointer"
                      >
                        Solicitar
                      </button>
                    </div>
              ...
              <div
                id="rma-history"
                className="scroll-mt-24 p-4 bg-slate-950 border border-slate-850 rounded-2xl space-y-3"
              >
              ...
              {/* FASE 3: Simulador de Contratación de Garantía Extendida */}
              <div className="p-4 bg-slate-950 border border-slate-850 rounded-2xl text-xs space-y-4">
                <div className="flex justify-between items-center pb-2 border-b border-slate-900">
                  <span className="text-[10px] text-slate-550 font-bold uppercase block text-left">
                    Simulador de Garantía Extendida y Coberturas
                  </span>
                  <span className="text-[10px] text-slate-400">
                    Estado:{" "}
                    <strong className="text-cyan-400 font-bold">
                      {WARRANTY_LABELS[selectedDevice.warrantyStatus]}
                    </strong>
                  </span>
                </div>
                ...
                <div className="flex flex-col sm:flex-row items-end gap-4">
                  <div className="flex-1 grid grid-cols-2 gap-3 text-left">
                    <div className="space-y-1">
                      ...
                      <select
                        value={warrantyYearsSim}
                        onChange={(e) =>
                          setWarrantyYearsSim(Number(e.target.value))
                        }
                        className="w-full bg-slate-900 border border-slate-800 rounded-xl px-2 py-1.5 text-[10px] text-white focus:outline-none"
                      >
                        <option value={1}>+1 Año Adicional ($18.500)</option>
                        <option value={2}>+2 Años Adicionales ($32.000)</option>
                        <option value={3}>+3 Años Todo Riesgo ($45.000)</option>
                      </select>
                    </div>
                    ...
                    <div className="space-y-1">
                      ...
                      <span className="px-3 py-1.5 bg-slate-900 border border-slate-800 rounded-xl text-[10px] font-bold text-slate-350 block font-mono">
                        $
                        {(warrantyYearsSim === 1
                          ? 18500
                          : warrantyYearsSim === 2
                  ...
                  <button
                    onClick={handleApplyWarrantyExtension}
                    className="px-4 py-2 bg-slate-900 hover:bg-slate-850 border border-slate-800 text-slate-300 font-bold rounded-xl text-[10px] uppercase cursor-pointer shrink-0"
                  >
                    Calcular cotización
                  </button>
                </div>
              ...
              <div className="p-4 bg-slate-950 border border-slate-850 rounded-2xl space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 border-t border-slate-900 pt-3">
                  ...
                  <div className="space-y-2">
                    <label className="text-[9px] text-slate-555 font-bold uppercase block">
            ...
            <div className="text-center py-28 text-slate-650 text-xs italic">
              Selecciona un dispositivo para iniciar RMA o validar su garantía
              oficial.
            </div>
          )}
        </div>
                ...
                <div className="flex flex-wrap gap-3 text-xs text-slate-400">
                  ...
                  <label className="flex items-center gap-1.5 cursor-pointer">
                    <input
                      type="checkbox"
                      checked={hasCables}
                      onChange={(e) => setHasCables(e.target.checked)}
                    />
                    Cables
                  </label>
                </div>
            ...
            <div className="flex gap-3">
              ...
              <button
                onClick={() => {
                  setFeedback({
                    tone: "success",
                    message: "Comprobante oficial enviado a impresión.",
                  });
