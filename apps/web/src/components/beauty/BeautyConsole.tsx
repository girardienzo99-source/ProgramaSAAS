...
const BEAUTY_STATUSES: BeautyAppointmentStatus[] = [
  "reserved",
  "confirmed",
  "in_progress",
  "completed",
...
export const BEAUTY_SERVICES: BeautyService[] = [
  ...
  {
    id: "s2",
    name: "Coloración / Tintura Completa",
    price: 25000,
    durationMins: 120,
  },
  {
    id: "s3",
    name: "Alisado Keratina Profesional",
    price: 32000,
    durationMins: 150,
  },
  {
    id: "s4",
    name: "Perfilado de Cejas + Depilación",
    price: 4500,
    durationMins: 30,
  },
];
...
const INITIAL_STYLISTS: Stylist[] = [
  {
    ...
    productCommissions: 1200,
  },
  {
    id: "st2",
    name: "Claudio Barber",
    commissionRate: 0.35,
    totalEarnedCommissions: 8500,
    productCommissions: 800,
  },
];
...
const INITIAL_APPTS: BeautyAppointment[] = [
  {
    ...
    stylistId: "st2",
    service: BEAUTY_SERVICES[0],
    time: "11:00",
    status: "reserved",
    colorFormula: "",
    appliedProducts: "Shampoo Neutro",
    chairOrBox: "Silla 2",
    depositAmount: 2000,
    paymentStatus: "señado",
    recommendedProducts: "Cera Fijadora Matte",
  },
  {
    id: "ba-2",
    clientName: "Clara Vignolo",
    stylistId: "st1",
    service: BEAUTY_SERVICES[1],
    time: "11:45",
    status: "in_progress",
    colorFormula: "Tono 7.3 + Oxidante 20vol (1:1)",
    appliedProducts: "Sérum Argan",
    chairOrBox: "Silla 1",
    depositAmount: 0,
    paymentStatus: "pending",
    recommendedProducts: "Shampoo Care Color",
  },
  {
    id: "ba-3",
    clientName: "Florencia Luna",
    stylistId: "st1",
    service: BEAUTY_SERVICES[3],
    time: "13:00",
    status: "completed",
    colorFormula: "",
    appliedProducts: "",
...
export default function BeautyConsole({
  services = BEAUTY_SERVICES,
  onServiceCompleted,
}: BeautyConsoleProps) {
  const [appointments, setAppointments] =
    useState<BeautyAppointment[]>(INITIAL_APPTS);
  const [stylists, setStylists] = useState<Stylist[]>(INITIAL_STYLISTS);
  const [selectedAppt, setSelectedAppt] = useState<BeautyAppointment | null>(
    INITIAL_APPTS[1],
  );
  ...
  const handleCalculateMix = (e: React.FormEvent) => {
    ...
    setFeedback({
      type: "success",
      message:
        "Fórmula química calculada y copiada al borrador de ficha técnica.",
    });
  };
  ...
  // Modal Comprobante
  const [printComprobante, setPrintComprobante] =
  ...
  const handleCreateAppointment = (e: React.FormEvent) => {
    ...
    setFeedback({
      type: "success",
      message: `Turno agendado en ${newChair} para ${newClient}.`,
    });
  };
  ...
  const handleCheckoutAppointment = (appt: BeautyAppointment) => {
    ...
    if (stylist) {
          ...
          s.id === stylist.id
            ? {
                ...
                totalEarnedCommissions:
                  s.totalEarnedCommissions + commissionAmount,
      ...
      setAppointments((prev) =>
        prev.map((a) =>
          a.id === appt.id
            ? { ...a, paymentStatus: "paid", status: "completed" }
            : a,
        ),
      );
      if (selectedAppt && selectedAppt.id === appt.id) {
        setSelectedAppt((prev) =>
          prev ? { ...prev, paymentStatus: "paid", status: "completed" } : null,
        );
      }
  ...
  const handleSaveTechnicalSheet = () => {
    ...
    setFeedback({
      type: "success",
      message: "Ficha técnica capilar y preferencias del cliente actualizadas.",
    });
  };
  ...
  const handleSendReminder = (appt: BeautyAppointment) => {
    ...
    setFeedback({
      type: "success",
      message: `Recordatorio preparado para ${appt.clientName}: "${text}". Falta conectar un proveedor de mensajería para enviarlo.`,
    });
  };
  ...
  const filteredAppointments = useMemo(
    () =>
      appointments.filter((appt) => {
        ...
        const matchStylist =
          stylistFilter === "all" || appt.stylistId === stylistFilter;
        const matchService =
          serviceFilter === "all" || appt.service.name === serviceFilter;
    ...
    <div className="w-full max-w-7xl mx-auto px-4 py-6 text-slate-100 flex flex-col gap-6 font-sans">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-black text-white">
              Consola de Salón & Fichas de Estilistas
            </h1>
            <p className="text-slate-400 text-xs mt-0.5">
              Control de agendas por silla, señas de turnos y comisiones de
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
        ...
        <div className="bg-slate-900/50 p-4 rounded-xl border border-slate-850">
          ...
          <span className="text-xl font-black text-indigo-400 mt-1 block">
            ...
            {stylists
              .reduce(
                (acc, curr) =>
                  acc + curr.totalEarnedCommissions + curr.productCommissions,
                0,
              )
              .toLocaleString()}
          </span>
        ...
        <div className="bg-slate-900/50 p-4 rounded-xl border border-slate-850">
          ...
          <span className="text-xs font-bold text-pink-400 mt-2 block truncate">
            Tintura Completa
          </span>
        </div>
      </div>
      ...
      {/* Alertas Rápidas */}
      {appointments.some(
        (a) => a.status === "reserved" && a.paymentStatus === "pending",
      ) && (
        <div className="p-4 bg-amber-550/10 border border-amber-500/25 rounded-2xl flex items-center justify-between">
          <div className="flex items-center gap-2.5 text-xs text-amber-300 font-semibold">
            ...
            <span>
              Alerta de Caja: Hay turnos programados próximos a ingresar sin
              seña de reserva registrada.
            </span>
          </div>
        </div>
      )}
      ...
      <div className="bg-slate-900/40 p-5 rounded-2xl border border-slate-850 grid grid-cols-1 sm:grid-cols-3 gap-3">
        ...
        <div className="space-y-1">
          ...
          >
            ...
            <option value="Coloración / Tintura Completa">
              Tintura Completa
            </option>
            <option value="Perfilado de Cejas + Depilación">
              Perfilado de Cejas
            </option>
          </select>
        </div>
                ...
                return (
                  <div
                    ...
                    className={`p-4 rounded-2xl text-left border cursor-pointer transition-all ${
                      active
                        ? "bg-pink-950/10 border-pink-500/40 text-white"
                  ...
                  >
                    ...
                    <span className="text-[10px] text-slate-400 block mt-1">
                      Estilista: {stylist?.name} | Hora: {appt.time}hs
                    </span>
                    ...
                    <div className="mt-3 flex justify-between items-center border-t border-slate-950 pt-2.5">
                      ...
                      <div className="flex gap-2">
                        <button
                          ...
                          aria-label={`Enviar recordatorio a ${appt.clientName}`}
                        ...
                        <button
                          ...
                          aria-label={`Imprimir comprobante de ${appt.clientName}`}
                          className="p-1 bg-slate-900 border border-slate-800 rounded text-slate-400 hover:text-white"
                        >
        ...
        <div className="lg:col-span-2 flex flex-col gap-6">
          ...
          {selectedAppt ? (
              ...
              <div className="flex justify-between items-start pb-4 border-b border-slate-850">
                <div>
                  ...
                  <span className="text-[10px] text-slate-500 block mt-0.5">
                    Servicio: {selectedAppt.service.name} | Ubicación:{" "}
                    {selectedAppt.chairOrBox} | Seña: $
                    {selectedAppt.depositAmount}
                  </span>
                </div>
              ...
              <div className="space-y-4">
                ...
                <div className="bg-slate-950 p-4 rounded-2xl border border-slate-850 space-y-3 text-xs">
                  <span className="text-[10px] text-pink-400 font-bold uppercase block">
                    Asistente Mezclador de Tintura & Decolorantes
                  </span>
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                    <div className="space-y-1">
                      ...
                      >
                        <option value="Castaño Oscuro (3)">
                          Castaño Oscuro (3)
                        </option>
                        <option value="Castaño Claro (5)">
                          Castaño Claro (5)
                        </option>
                        <option value="Rubio Oscuro (6)">
                          Rubio Oscuro (6)
                        </option>
                    ...
                    <div className="space-y-1">
                      ...
                      <select
                        ...
                        className="w-full bg-slate-900 border border-slate-800 rounded-xl px-2 py-1.5 text-xs text-white"
                      >
                        <option value="Rubio Dorado (8.3)">
                          Rubio Dorado (8.3)
                        </option>
                        <option value="Cobrizo Claro (7.4)">
                          Cobrizo Claro (7.4)
                        </option>
                        <option value="Platinado Ultra (10.1)">
                          Platinado Ultra (10.1)
                        </option>
                      </select>
                    </div>
            ...
            <div className="bg-slate-900/40 p-10 rounded-3xl border border-slate-850 text-center text-slate-650 text-xs italic">
              Selecciona un turno para registrar coloración capilar o
              comisiones.
            </div>
          )}
          ...
          <div className="bg-slate-900/40 p-6 rounded-3xl border border-slate-850 space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {stylists.map((s) => (
                <div
                  key={s.id}
                  className="p-4 bg-slate-950 border border-slate-850 rounded-2xl space-y-2"
                >
                  ...
                  <div className="grid grid-cols-2 gap-2 text-[10px] text-slate-500 border-t border-slate-900 pt-2">
                    <div>
                      ...
                      <span className="font-bold text-slate-350 block mt-0.5">
                        ${s.totalEarnedCommissions.toLocaleString()}
                      </span>
                    </div>
                    <div>
                      <span>Comisión Productos</span>
                      <span className="font-bold text-slate-350 block mt-0.5">
                        ${s.productCommissions.toLocaleString()}
                      </span>
                    </div>
                  </div>
                </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              {CHAIRS.map((chair) => {
                ...
                return (
                  <div
                    key={chair}
                    className="p-4 bg-slate-950 border border-slate-850 rounded-2xl flex flex-col justify-between min-h-[120px] text-xs gap-3 text-left"
                  >
                    <div>
                      <div className="flex justify-between items-center">
                        ...
                        <span
                          className={`inline-flex px-1.5 py-0.5 rounded text-[8px] font-bold uppercase ${
                            isOccupied
                              ? "bg-pink-500/10 text-pink-450 border border-pink-500/20"
                              : isReserved
                      ...
                      {activeAppt ? (
                        <div className="space-y-1 mt-2">
                          <span className="text-[10px] text-slate-350 block font-bold truncate">
                            Huésped: {activeAppt.clientName}
                          </span>
                          <span className="text-[9px] text-slate-500 block truncate">
                            Servicio: {activeAppt.service.name}
                          </span>
                          <span className="text-[9px] text-slate-500 block font-mono">
                            Hora: {activeAppt.time} hs
                          </span>
                        </div>
                      ) : (
                        <span className="text-[10px] text-slate-650 italic block mt-2">
                          Sillón libre
                        </span>
                      )}
                    </div>
        ...
        <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white border border-slate-200 rounded-3xl p-6 max-w-sm w-full space-y-6 shadow-2xl text-slate-900 animate-fade-in">
            <div className="flex justify-between items-start border-b border-slate-100 pb-3">
              ...
              <button
                onClick={() => setPrintComprobante(null)}
            ...
            <button
              onClick={() => {
                setFeedback({
                  type: "success",
                  message: "Comprobante de turno enviado a impresión.",
                });
