...
type PropertyType =
  "Casa" | "Departamento" | "Local" | "Terreno" | "Galpón" | "Oficina";
...
const PROPERTY_TYPE_FILTERS: Array<{
  label: string;
...
const OFFER_STATUS_OPTIONS: Array<{ label: string; value: OfferStatus }> =
...
const INITIAL_PROPERTIES: Property[] = [
  ...
  },
  {
    ...
    offers: [
      {
        id: "o3",
        offererName: "Paula Albarracín",
        offerAmount: 620000,
        date: "2026-07-01",
        status: "pending",
      },
    ],
    visits: [{ visitorName: "Ramiro Díaz", date: "2026-07-02", time: "11:00" }],
  },
];
...
const INITIAL_CLAIMS: MaintenanceClaim[] = [
  {
    id: "c-101",
    propertyAddress: "Av. Santa Fe 3400 5°B, CABA",
    description: "Pérdida de agua bajo mesada de cocina.",
    status: "pending",
  },
  {
    id: "c-102",
    propertyAddress: "Corrientes 1500, Local Bajo",
    description: "Falla en persiana metálica motorizada.",
    status: "scheduled",
  },
];
...
export default function RealEstateConsole() {
  ...
  const handleCreateProperty = (e: React.FormEvent) => {
    ...
    if (
      ...
      setFeedback({
        tone: "error",
        message:
          "Completá dirección, propietario, alquiler y superficie con valores válidos.",
      });
      return;
    }
    ...
    setFeedback({
      tone: "success",
      message: "Propiedad y contrato cargados exitosamente.",
    });
  };
  ...
  const handleCreateOffer = (e: React.FormEvent) => {
    ...
    if (!selectedProperty || !offerer || amount <= 0) {
      setFeedback({
        tone: "error",
        message: "Completá interesado y monto válido para registrar la oferta.",
      });
      return;
    }
    ...
    const updated: Property = {
      ...selectedProperty,
      offers: [...selectedProperty.offers, newOffer],
    };
  ...
  const handleOfferStatusChange = (offerId: string, status: OfferStatus) => {
    ...
    const updated: Property = {
      ...selectedProperty,
      offers: selectedProperty.offers.map((o) =>
    ...
    setFeedback({
      tone: "success",
      message: `Oferta marcada como ${OFFER_STATUS_LABELS[status].toLowerCase()}.`,
    });
  };

  const handleContractStateChange = (id: string, state: ContractState) => {
    setProperties((prev) =>
      prev.map((p) => (p.id === id ? { ...p, contractState: state } : p)),
    );
    if (selectedProperty && selectedProperty.id === id) {
      setSelectedProperty((prev) =>
        prev ? { ...prev, contractState: state } : null,
      );
    }
    setFeedback({
      tone: "success",
      message: `Contrato actualizado a ${CONTRACT_STATE_LABELS[state]}.`,
    });
  };
  ...
  const handleAddClaim = (e: React.FormEvent) => {
    ...
    if (!selectedProperty || !description) {
      setFeedback({
        tone: "error",
        message: "Escribí el detalle del reclamo antes de cargarlo.",
      });
      return;
    }
    ...
    setFeedback({
      tone: "success",
      message: "Reclamo de mantenimiento registrado. Se asignará técnico.",
    });
  };
  ...
  const handleUpdateClaimStatus = (id: string, status: ClaimStatus) => {
    ...
    setFeedback({
      tone: "success",
      message: `Reclamo actualizado a ${CLAIM_STATUS_LABELS[status].toLowerCase()}.`,
    });
  };
  ...
  const metrics = useMemo(() => {
    ...
    return {
      ...
      arrearsTotal: properties
        .filter((p) => p.status === "arrears")
        .reduce((acc, curr) => acc + curr.rentAmount, 0),
  ...
  const filteredProperties = useMemo(
    () =>
      properties.filter((p) => {
        ...
        const matchType =
          propertyTypeFilter === "all" || p.type === propertyTypeFilter;
        const matchOp =
          operationFilter === "all" || p.operation === operationFilter;
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-black text-white">
              Administración de Contratos & Alquileres
            </h1>
            <p className="text-slate-400 text-xs mt-0.5">
              Control de expensas, liquidaciones a propietarios y reclamos de
              mantenimiento.
            </p>
          </div>
        </div>
      ...
      {properties.some((p) => p.contractState === "expired") && (
        <div className="p-4 bg-amber-550/10 border border-amber-500/25 rounded-2xl flex items-center justify-between">
          <div className="flex items-center gap-2.5 text-xs text-amber-300 font-semibold">
            ...
            <span>
              Alerta Inmobiliaria: Existen contratos de alquiler vencidos que
              requieren renovación o rescisión formal.
            </span>
          </div>
        </div>
      )}
      ...
      <div className="bg-slate-900/40 p-5 rounded-2xl border border-slate-850 grid grid-cols-1 sm:grid-cols-3 gap-3">
        <div className="space-y-1">
          <label className="text-[9px] text-slate-500 font-bold uppercase block">
            Buscar Inmueble / Dueño / Inquilino
          </label>
        ...
        <div className="space-y-1">
          ...
          <select
            value={propertyTypeFilter}
            onChange={(e) => {
              if (isPropertyTypeFilter(e.target.value))
        ...
        <div className="space-y-1">
          ...
          <select
            ...
            onChange={(e) => {
              if (isOperationFilter(e.target.value))
                ...
                return (
                  <div
                    ...
                    className={`p-4 rounded-2xl text-left border cursor-pointer transition-all ${
                      active
                        ? "bg-yellow-950/10 border-yellow-500/40 text-white"
        ...
        <div className="lg:col-span-2 flex flex-col gap-6">
          {selectedProperty ? (
            <div className="bg-slate-900/40 p-6 rounded-3xl border border-slate-850 space-y-6">
              <div className="flex justify-between items-start pb-4 border-b border-slate-850">
                <div>
                  ...
                  <span className="text-[10px] text-slate-500 block mt-0.5">
                    Propietario: {selectedProperty.ownerName} | Inquilino:{" "}
                    {selectedProperty.tenantName} | Superficie:{" "}
              ...
              <div className="p-3 bg-yellow-950/20 border border-yellow-500/20 rounded-2xl flex items-center justify-between text-xs">
                <div className="flex items-center gap-2">
                  <Building2 className="h-4 w-4 text-yellow-400" />
                  <div>
                    <span className="font-bold text-slate-200 block">
                      Liquidación Mensual de Expensas e Impuestos
                    </span>
                    <span className="text-[10px] text-slate-400">
                      Estimación: alquiler $
                      {selectedProperty.rentAmount.toLocaleString()} + expensas
                      ${selectedProperty.expensasAmount.toLocaleString()} = $
                      {(
                        selectedProperty.rentAmount +
                        selectedProperty.expensasAmount
                      ).toLocaleString()}
                      . No genera una liquidación contable.
                    </span>
                  </div>
                </div>
                <span className="px-2 py-1 bg-yellow-500/10 text-yellow-400 text-[10px] font-bold rounded-lg border border-yellow-500/20 uppercase">
                  Vista previa
                </span>
              </div>
              ...
              <div className="p-4 bg-slate-950 border border-slate-850 rounded-2xl space-y-4">
                <span className="text-[10px] text-slate-555 font-bold uppercase block">
                  Parámetros del Contrato & Ubicación
                </span>
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-xs">
                  <div className="p-3 bg-slate-900 border border-slate-800 rounded-xl">
                    ...
                    <span className="font-bold text-slate-300">
                      {selectedProperty.contractStart}
                    </span>
                  </div>
                  <div className="p-3 bg-slate-900 border border-slate-800 rounded-xl">
                    <span className="text-[9px] text-slate-500 font-bold uppercase block">
                ...
                <div className="flex justify-between items-center pt-2 border-t border-slate-900">
                  ...
                  <div className="flex bg-slate-900 p-0.5 rounded-lg border border-slate-850 text-[10px]">
                    <button
                      onClick={() =>
                        handleContractStateChange(selectedProperty.id, "draft")
                      }
                      className={`px-2 py-0.5 rounded ${selectedProperty.contractState === "draft" ? "bg-yellow-600 text-slate-950 font-bold" : "text-slate-500"}`}
                    >
                      Borrador
                    </button>
                    <button
                      onClick={() =>
                        handleContractStateChange(selectedProperty.id, "active")
                      }
                      className={`px-2 py-0.5 rounded ${selectedProperty.contractState === "active" ? "bg-emerald-600 text-slate-950 font-bold" : "text-slate-500"}`}
                    >
                      Activo
                    </button>
                    <button
                      ...
                      className={`px-2 py-0.5 rounded ${selectedProperty.contractState === "expired" ? "bg-amber-600 text-slate-950 font-bold" : "text-slate-500"}`}
                    >
                      Vencido
                    </button>
                    <button
                      ...
                      className={`px-2 py-0.5 rounded ${selectedProperty.contractState === "terminated" ? "bg-rose-600 text-slate-950 font-bold" : "text-slate-500"}`}
                    >
                      Rescindido
                    </button>
                  </div>
              ...
              {selectedProperty.contractState === "active" && (
                <div className="p-4 bg-slate-950 border border-slate-850 rounded-2xl space-y-3">
                  <span className="text-xs font-bold text-yellow-500 uppercase block">
                    Calcular & Aplicar Indexación (Ajuste Anual)
                  </span>

                  <p className="text-[10px] text-slate-500">
                    Ajustá el importe del alquiler mensual utilizando el índice
                    acumulado ICL/IPC pactado.
                  </p>

                  <form
                    onSubmit={handleApplyIndexation}
                    ...
                    <div className="flex-1 flex gap-2">
                      <div className="flex-1 space-y-1">
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            setIndexRatePercent(Number(e.target.value))
                          }
              ...
              <div className="p-4 bg-slate-950 border border-slate-850 rounded-2xl space-y-4">
                <span className="text-[10px] text-slate-555 font-bold uppercase block">
                  Historial de Ofertas Económicas
                </span>

                <div className="space-y-2 max-h-24 overflow-y-auto">
                  {selectedProperty.offers.length === 0 ? (
                    <span className="text-xs text-slate-600 italic block">
                      No hay ofertas económicas para este inmueble.
                    </span>
                  ) : (
                    selectedProperty.offers.map((o) => (
                      <div
                        key={o.id}
                        className="flex justify-between items-center text-xs bg-slate-900/40 p-2 rounded-xl border border-slate-900"
                      >
                        <div>
                          <span className="font-bold text-slate-200 block">
                            {o.offererName}
                        ...
                        <div className="flex gap-3 items-center">
                          ...
                          <select
                            value={o.status}
                            onChange={(e) => {
                              if (isOfferStatus(e.target.value))
                                handleOfferStatusChange(o.id, e.target.value);
                            }}
                ...
                <form
                  ...
                  className="grid grid-cols-1 sm:grid-cols-3 gap-3 pt-3 border-t border-slate-900"
                >
                  <input
                    type="text"
                    required
                  ...
                  <button
                    type="submit"
                    className="py-1.5 bg-yellow-600 hover:bg-yellow-500 text-slate-950 font-bold rounded-xl text-xs"
                  >
                    Registrar Oferta
                  </button>
                </form>
              ...
              <form
                ...
                className="space-y-3 pt-4 border-t border-slate-900"
              >
                ...
                <div className="flex gap-2">
                  ...
                  <button
                    type="submit"
                    className="px-4 bg-slate-950 border border-slate-800 text-slate-350 hover:bg-slate-900 rounded-xl text-xs"
                  >
                    Cargar Reclamo
                  </button>
                </div>
          ...
          <div className="bg-slate-900/40 p-6 rounded-3xl border border-slate-850 space-y-4">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 border-b border-slate-900 pb-3">
              ...
              <div className="flex items-center gap-2 text-xs">
                ...
                >
                  <option value="Mario Pereyra (Electricista)">
                    Mario Pereyra (Electricista)
                  </option>
                  <option value="Juan Gómez (Plomero/Gasista)">
                    Juan Gómez (Plomero/Gasista)
                  </option>
                  <option value="Lucas Altieri (Cerrajero)">
                    Lucas Altieri (Cerrajero)
                  </option>
                  <option value="Roberto Díaz (Pintor/Albañil)">
                    Roberto Díaz (Pintor/Albañil)
                  </option>
                </select>
              </div>
            </div>
            ...
            <div className="space-y-2 max-h-40 overflow-y-auto pr-1">
              {claims.map((c) => (
                <div
                  key={c.id}
                  className="p-3 bg-slate-950 border border-slate-850 rounded-xl flex justify-between items-center text-xs"
                >
                  <div>
                    <span className="font-extrabold text-slate-300 block text-[11px]">
                      {c.propertyAddress}
                    </span>
                  ...
                  <div className="flex gap-2.5 items-center">
                    ...
                    <select
                      value={c.status}
                      onChange={(e) => {
                        if (isClaimStatus(e.target.value))
                          handleUpdateClaimStatus(c.id, e.target.value);
                      }}
                      className="bg-slate-900 border border-slate-800 text-[10px] rounded px-1.5 py-0.5 text-slate-350"
                    >
            ...
            <div className="flex gap-3">
              ...
              <button
                onClick={() => {
                  setFeedback({
                    tone: "success",
                    message: "Comprobante inmobiliario enviado a impresión.",
                  });
