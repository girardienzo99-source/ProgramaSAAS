...
import { 
  Wrench, Car, Clock, User, Plus, Trash2, CheckCircle2, 
  ChevronRight, X, ClipboardList, AlertTriangle, ShieldCheck,
  Search, Printer, Check, Hammer, HelpCircle, Phone, Award,
  Camera, PenTool, Layers, DollarSign
} from 'lucide-react';

type StockStatus = 'available' | 'ordered' | 'out_of_stock';
type RepairStatus = 'entered' | 'diagnostic' | 'waiting_parts' | 'repairing' | 'ready' | 'delivered';
type BudgetStatus = 'pending' | 'approved' | 'rejected';
type Feedback = { type: 'success' | 'error'; message: string } | null;

const REPAIR_STATUSES: RepairStatus[] = ['entered', 'diagnostic', 'waiting_parts', 'repairing', 'ready', 'delivered'];
...
export const SPARE_PARTS: SparePart[] = [
  { id: 'p1', name: 'Pastillas de Freno Delanteras Remsa', price: 18500, stockStatus: 'available', compatibility: 'Toyota' },
  { id: 'p2', name: 'Filtro de Aceite Fram', price: 5400, stockStatus: 'available', compatibility: 'Volkswagen' },
  { id: 'p3', name: 'Aceite Semisintético Elaion 10W40 4L', price: 29000, stockStatus: 'available', compatibility: 'Todas' },
  { id: 'p4', name: 'Bujía de Encendido NGK', price: 3200, stockStatus: 'ordered', compatibility: 'Toyota' }
];
...
const INITIAL_ORDERS: RepairOrder[] = [
  ...
  },
  {
    ...
    partsUsed: [{ part: SPARE_PARTS[1], qty: 1 }, { part: SPARE_PARTS[2], qty: 1 }],
    laborCost: 8000,
    status: 'diagnostic',
...
export default function AutomotiveConsole({ parts = SPARE_PARTS, onPartUsed }: AutomotiveConsoleProps) {
  const [orders, setOrders] = useState<RepairOrder[]>(INITIAL_ORDERS);
  const [selectedOrder, setSelectedOrder] = useState<RepairOrder | null>(INITIAL_ORDERS[0]);
  ...
  const handleCreateOrder = (e: React.FormEvent) => {
    ...
    setFeedback({ type: 'success', message: `Orden ${newOrder.id.toUpperCase()} creada para ${newOrder.plate}.` });
  ...
  const handleToggleDamageZone = (zone: string) => {
    ...
    setFeedback({ type: 'success', message: `Zona "${zone}" actualizada en el peritaje de la orden.` });
  };
  ...
  const handleAddPartToOrder = () => {
    ...
    setOrders(prev => prev.map(o => {
      if (o.id === selectedOrder.id) {
        ...
        const existingIdx = updatedParts.findIndex(x => x.part.id === part.id);
        if (existingIdx > -1) {
  ...
  const handleBudgetStatusChange = (id: string, budgetStatus: BudgetStatus) => {
    setOrders(prev => prev.map(o => o.id === id ? { ...o, budgetStatus } : o));
  ...
  const calculateTotal = (order: RepairOrder) => {
    const partsTotal = order.partsUsed.reduce((acc, curr) => acc + (curr.part.price * curr.qty), 0);
    return partsTotal + order.laborCost;
  };

  const handlePrintOT = (order: RepairOrder) => {
    const partsList = order.partsUsed.map(p => `${p.part.name} (x${p.qty})`).join(', ');
  ...
  const handlePrintPresupuesto = (order: RepairOrder) => {
    ...
    const partsText = order.partsUsed.map(p => `- ${p.part.name} x${p.qty}: $${(p.part.price * p.qty).toLocaleString()}`).join('\n');
  ...
  const filteredOrders = useMemo(() => orders.filter(o => {
    const matchSearch = o.plate.toLowerCase().includes(searchTerm.toLowerCase()) || 
                        o.vehicle.toLowerCase().includes(searchTerm.toLowerCase()) || 
                        o.client.toLowerCase().includes(searchTerm.toLowerCase());
    const matchMechanic = mechanicFilter === 'all' || o.mechanic === mechanicFilter;
    ...
    <div className="w-full max-w-7xl mx-auto px-4 py-6 text-slate-100 flex flex-col gap-6 font-sans">
      ...
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center bg-slate-900/60 p-5 rounded-2xl border border-slate-800 gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-black text-white">Gestión de Órdenes de Trabajo (Taller)</h1>
            <p className="text-slate-400 text-xs mt-0.5">Control de checklist, presupuestos con firma de aceptación y trazabilidad de repuestos.</p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
        <div className="bg-slate-900/50 p-4 rounded-xl border border-slate-850">
          <span className="text-[10px] text-slate-500 font-bold uppercase block">Vehículos Ingresados</span>
          <span className="text-xl font-black text-white mt-1 block">{orders.length}</span>
        </div>
        <div className="bg-slate-900/50 p-4 rounded-xl border border-slate-850">
          <span className="text-[10px] text-slate-500 font-bold uppercase block">En Reparación</span>
          <span className="text-xl font-black text-amber-450 mt-1 block">
            {orders.filter(o => o.status === 'repairing').length}
          </span>
        </div>
        <div className="bg-slate-900/50 p-4 rounded-xl border border-slate-850">
          <span className="text-[10px] text-slate-500 font-bold uppercase block">Presupuestos Rechazados</span>
          <span className="text-xl font-black text-rose-500 mt-1 block">
            {orders.filter(o => o.budgetStatus === 'rejected').length}
          </span>
        </div>
        <div className="bg-slate-900/50 p-4 rounded-xl border border-slate-850">
          <span className="text-[10px] text-slate-500 font-bold uppercase block">Mano de Obra Hoy</span>
          <span className="text-xl font-black text-emerald-450 mt-1 block">
            ${orders.reduce((acc, curr) => acc + curr.laborCost, 0).toLocaleString()}
          </span>
        </div>
        <div className="bg-slate-900/50 p-4 rounded-xl border border-slate-850">
          <span className="text-[10px] text-slate-500 font-bold uppercase block">Mecánico Líder</span>
          <span className="text-xs font-bold text-cyan-400 mt-2 block truncate">Mario Pereyra (1)</span>
        </div>
      </div>
      ...
      {orders.some(o => o.budgetStatus === 'pending') && (
        <div className="p-4 bg-amber-550/10 border border-amber-500/25 rounded-2xl flex items-center justify-between">
          <div className="flex items-center gap-2.5 text-xs text-amber-300 font-semibold">
            ...
            <span>Alerta de Taller: Hay vehículos parados a la espera de la aprobación del presupuesto del cliente.</span>
          </div>
        </div>
      )}
      ...
      <div className="bg-slate-900/40 p-5 rounded-2xl border border-slate-850 grid grid-cols-1 sm:grid-cols-3 gap-3">
        <div className="space-y-1">
          <label className="text-[9px] text-slate-500 font-bold uppercase block">Buscar por Patente / Auto / Cliente</label>
        ...
        <div className="space-y-1">
          <label className="text-[9px] text-slate-500 font-bold uppercase block">Mecánico Asignado</label>
          <select
        ...
        <div className="space-y-1">
          <label className="text-[9px] text-slate-500 font-bold uppercase block">Estado de OT</label>
          <select
            value={statusFilter}
            onChange={e => {
                ...
                return (
                  ...
                  >
                    <div className="flex justify-between items-center w-full">
                      <span className="font-extrabold text-xs text-white">{order.vehicle}</span>
                    ...
                    <div className="flex justify-between items-center text-[10px] text-slate-500 mt-2">
                      ...
                      <span className={`px-2 py-0.5 rounded text-[8px] font-bold uppercase ${
                        order.budgetStatus === 'approved' ? 'bg-emerald-500/10 text-emerald-450 border border-emerald-500/20' :
                        order.budgetStatus === 'rejected' ? 'bg-rose-500/10 text-rose-400 border border-rose-500/20' :
          ...
          {selectedOrder ? (
            <>
              ...
              <div className="flex justify-between items-start pb-4 border-b border-slate-850">
                <div>
                  <h4 className="font-black text-base text-white">{selectedOrder.vehicle} ({selectedOrder.year})</h4>
                  <span className="text-[10px] text-slate-500 block mt-0.5">
                    Cliente: {selectedOrder.client} | KM: {selectedOrder.km.toLocaleString()} | Patente: {selectedOrder.plate}
                  </span>
                </div>
              ...
              <div className="bg-slate-950 p-5 rounded-2xl border border-slate-850 space-y-4">
                ...
                <p className="text-[10px] text-slate-500 leading-normal">
                  Hacé click en cualquier zona del croquis para registrar rasguños/golpes detectados durante la recepción.
                </p>
                ...
                <div className="grid grid-cols-4 gap-3 text-center">
                  {[
                    ...
                    { zone: 'Costado Izquierdo (Golpe)', label: 'Lat. Izquierdo' },
              ...
              <div className="p-4 bg-slate-950 border border-slate-850 rounded-2xl space-y-3">
                <span className="text-[10px] text-slate-550 font-bold uppercase block">Timeline / Estado de Reparación</span>
                ...
                <div className="p-3.5 bg-slate-950 border border-slate-850 rounded-xl space-y-1.5 text-xs">
                  ...
                  <div className="flex gap-2 pt-1">
                    {selectedOrder.photos.map((f, i) => (
                      <span key={i} className="px-2.5 py-1 bg-slate-900 border border-slate-800 rounded-lg text-[10px] text-slate-350 cursor-pointer hover:border-slate-600">
                        {f}
                      </span>
                    ))}
                  </div>
              ...
              <div className="p-4 bg-slate-950 border border-slate-850 rounded-2xl space-y-4">
                <span className="text-xs font-bold text-cyan-400 uppercase block">Repuestos & Trazabilidad de Stock</span>

                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-2 border-t border-slate-900">
                  <div className="space-y-2">
                    <label className="text-[9px] text-slate-550 font-bold uppercase block">Seleccionar Repuesto</label>
                    <div className="flex gap-2">
                      <select
                      ...
                      >
                        {parts.map(part => (
                          <option key={part.id} value={part.id}>{part.name} (Compatibilidad: {part.compatibility || 'Todas'})</option>
                        ))}
                      </select>
                      <button onClick={handleAddPartToOrder} className="px-3 bg-cyan-600 hover:bg-cyan-500 text-slate-950 font-bold rounded-xl text-xs">+</button>
                    </div>
                    {(() => {
                      const selectedPart = parts.find(p => p.id === selectedPartId);
                      const isCompatible = selectedPart && (selectedPart.compatibility === 'Todas' || selectedPart.compatibility === selectedOrder.brand);
                      if (!isCompatible && selectedPart) {
                        return (
                          <span className="text-[9px] text-amber-500 font-bold block pt-1 animate-pulse">
                            ⚠️ Incompatibilidad: Ficha técnica ({selectedPart.compatibility}) difiere de la marca del auto ({selectedOrder.brand}).
                          </span>
                        );
                      }
                      return null;
                  ...
                  <div className="space-y-2">
                    <label className="text-[9px] text-slate-550 font-bold uppercase block">Mano de Obra ($)</label>
                    <div className="flex gap-2">
                      <input
                        type="number"
                      ...
                      />
                      <button onClick={handleUpdateLabor} className="px-3 bg-slate-800 hover:bg-slate-700 text-slate-350 font-bold rounded-xl text-xs border border-slate-850">Aplicar</button>
                    </div>
                  </div>
                </div>
                ...
                <div className="space-y-2 pt-3 border-t border-slate-900">
                  <span className="text-[10px] text-slate-500 font-bold uppercase block">Repuestos Utilizados</span>
                  {selectedOrder.partsUsed.map((u, i) => (
                    <div key={i} className="flex justify-between items-center text-xs bg-slate-900/40 p-2 rounded-lg border border-slate-900">
                      <span>{u.part.name} x{u.qty}</span>
                      <div className="flex gap-3 items-center">
                        <span className={`px-1.5 py-0.5 rounded text-[8px] font-bold uppercase ${
                          u.part.stockStatus === 'available' ? 'bg-emerald-500/10 text-emerald-450' : 'bg-rose-500/10 text-rose-400'
                        }`}>
                          {u.part.stockStatus}
                        </span>
                        <span className="font-bold">${(u.part.price * u.qty).toLocaleString()}</span>
                      </div>
                    </div>
                  ))}
                </div>
                ...
                <div className="flex justify-between items-center pt-3 border-t border-slate-900">
                  <div>
                    <span className="text-[9px] text-slate-555 font-bold uppercase block">Aprobación Cliente</span>
                    <div className="flex bg-slate-900 p-0.5 rounded-lg border border-slate-850 text-[10px] mt-1">
                      <button onClick={() => handleBudgetStatusChange(selectedOrder.id, 'approved')} className={`px-2 py-0.5 rounded ${selectedOrder.budgetStatus === 'approved' ? 'bg-emerald-600 text-slate-950 font-bold' : 'text-slate-500'}`}>Aprobado</button>
                      <button onClick={() => handleBudgetStatusChange(selectedOrder.id, 'pending')} className={`px-2 py-0.5 rounded ${selectedOrder.budgetStatus === 'pending' ? 'bg-amber-600 text-slate-950 font-bold' : 'text-slate-500'}`}>Pendiente</button>
                      <button onClick={() => handleBudgetStatusChange(selectedOrder.id, 'rejected')} className={`px-2 py-0.5 rounded ${selectedOrder.budgetStatus === 'rejected' ? 'bg-rose-600 text-slate-950 font-bold' : 'text-slate-500'}`}>Rechazado</button>
                    </div>
                  </div>
                  <div className="text-right">
                    <span className="text-[9px] text-slate-555 font-bold uppercase block">Importe Estimado</span>
                    <span className="text-xl font-black text-cyan-400">${calculateTotal(selectedOrder).toLocaleString()}</span>
                  </div>
                </div>
              </div>
        ...
        <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white border border-slate-200 rounded-3xl p-6 max-w-md w-full space-y-6 shadow-2xl text-slate-900 animate-fade-in">
            ...
            <div className="flex gap-3">
              ...
              <button
                onClick={() => {
                  setFeedback({ type: 'success', message: 'Comprobante de taller enviado a impresión.' });
      ...
      {isAddModalOpen && (
        <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 w-full max-w-lg shadow-2xl">
            <h2 className="text-xl font-black mb-4 text-white">Nueva Orden de Trabajo</h2>
            <form onSubmit={handleCreateOrder} className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="text-[10px] text-slate-500 uppercase font-bold block mb-1">Patente / Dominio</label>
                  <input
                    type="text"
                    required
                ...
                <div>
                  <label className="text-[10px] text-slate-500 uppercase font-bold block mb-1">Marca</label>
                  <input
                    type="text"
                    required
              ...
              <div className="grid grid-cols-3 gap-4">
                <div className="col-span-2">
                  <label className="text-[10px] text-slate-500 uppercase font-bold block mb-1">Modelo / Versión</label>
                  <input
                    type="text"
                    required
                ...
                <div>
                  <label className="text-[10px] text-slate-500 uppercase font-bold block mb-1">Año</label>
                  <input
                    type="number"
                    required
                    value={newYear}
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="text-[10px] text-slate-500 uppercase font-bold block mb-1">Kilometraje</label>
                  <input
                    type="number"
                    required
                    value={newKm}
