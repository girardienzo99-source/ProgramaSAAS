...
type RoomStatus =
  "free" | "reserved" | "occupied" | "cleaning" | "maintenance" | "blocked";
...
const INITIAL_ROOMS: Room[] = [
  {
    ...
    type: "Standard",
    status: "occupied",
    guestName: "Horacio Quiroga",
    guestPassport: "ARG-35123456",
    guestPhone: "11-5841-2532",
    guestCount: 2,
    securityDepositPaid: true,
    checkInDate: "2026-07-08",
    checkOutDate: "2026-07-12",
    ratePerNight: 45000,
    extras: [{ item: "Agua Minibar", price: 1500 }],
    housekeeperAssigned: "Marta Cleaning",
  ...
  },
  {
    ...
    status: "reserved",
    guestName: "Clara Vignolo",
    guestPassport: "ARG-40987654",
    guestPhone: "11-9988-7733",
    guestCount: 1,
...
export default function HotelConsole() {
  ...
  const handleCheckIn = (e: React.FormEvent) => {
    ...
    if (!selectedRoom || !guest || guestCount <= 0 || !checkOutDate) {
      setFeedback({
        tone: "error",
        message:
          "Completá huésped, cantidad y fecha de salida para hacer check-in.",
      });
      return;
    }
    ...
    setFeedback({
      tone: "success",
      message: `Check-in confirmado en habitación ${selectedRoom.number} para ${guest}.`,
    });
  };
  ...
  const handleAddExtra = () => {
    ...
    if (!extraItem.trim() || price <= 0) {
      setFeedback({
        tone: "error",
        message: "Seleccioná un consumo con precio válido.",
      });
      return;
    }
    ...
    setFeedback({
      tone: "success",
      message: `Cargo extra agregado: ${extraItem} ($${price.toLocaleString()}).`,
    });
  };
  ...
  const handleCheckOut = () => {
    ...
    const extrasTotal = selectedRoom.extras.reduce(
    ...
    setFeedback({
      tone: "success",
      message: `Check-out finalizado para ${selectedRoom.guestName}. Total facturado: $${totalToPay.toLocaleString()}.`,
    });
  };
  ...
  const handleCleanRoom = () => {
    ...
    setFeedback({
      tone: "success",
      message: `Habitación ${selectedRoom.number} limpia y lista para recibir nuevos huéspedes.`,
    });
  };
  ...
  const handlePrintFolioInvoice = (room: Room) => {
    ...
    const extrasText = room.extras
      .map((e) => `- ${e.item}: $${e.price.toLocaleString()}`)
      .join("\n");
  ...
  const metrics = useMemo(() => {
    ...
    const extrasTotal = rooms.reduce(
      (acc, curr) => acc + curr.extras.reduce((a, b) => a + b.price, 0),
      0,
    );
    ...
    return {
      occupiedCount,
      occupancyRate:
        rooms.length > 0 ? (occupiedCount / rooms.length) * 100 : 0,
      freeRooms: rooms.filter((r) => r.status === "free").length,
      blockedOrCleaning: rooms.filter(
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-black text-white">
              Rack de Ocupación & Minibar (Front Desk)
            </h1>
            <p className="text-slate-400 text-xs mt-0.5">
              Control de check-in, check-out, cargos extras al folio de
              habitación y estado de limpieza.
            </p>
          </div>
        </div>
      </div>
      ...
      {rooms.some((r) => r.status === "occupied" && !r.securityDepositPaid) && (
        <div className="p-4 bg-rose-550/10 border border-rose-500/25 rounded-2xl flex items-center justify-between">
          <div className="flex items-center gap-2.5 text-xs text-rose-350 font-semibold">
            ...
            <span>
              Alerta de Conserjería: Existen habitaciones ocupadas sin tarjeta
              de crédito cargada en depósito de garantía.
            </span>
          </div>
        </div>
      )}
      ...
      <div className="bg-slate-900/40 p-5 rounded-2xl border border-slate-850 grid grid-cols-1 sm:grid-cols-3 gap-3">
        ...
        <div className="space-y-1">
          ...
          <select
            ...
            onChange={(e) => {
              if (isRoomTypeFilter(e.target.value))
        ...
        <div className="space-y-1">
          ...
          <select
            ...
            onChange={(e) => {
              if (isRoomStatusFilter(e.target.value))
        ...
        <div className="lg:col-span-2 bg-slate-900/40 p-6 rounded-3xl border border-slate-850 flex flex-col justify-between min-h-[480px]">
          {selectedRoom ? (
            <div className="space-y-6">
                  ...
                  {selectedRoom.status === "occupied" && (
                    <>
                      <button
                        ...
                        aria-label={`Imprimir comprobante de habitación ${selectedRoom.number}`}
              ...
              {selectedRoom.status === "cleaning" && (
                <div className="p-4 bg-slate-950 border border-slate-850 rounded-2xl text-xs space-y-3.5">
                  <span className="text-[10px] text-slate-550 font-bold uppercase block">
                    Auditoría e Inspección de Calidad de Limpieza
                  </span>

                  <p className="text-[10px] text-slate-500">
                    Completá los puntos de control de mucamas para habilitar y
                  ...
                  <button
                    ...
                    disabled={
                      !(
                        cleaningChecklist.sheets &&
                        cleaningChecklist.minibar &&
                        cleaningChecklist.sanitized
                      )
                    }
              ...
              {selectedRoom.status === "occupied" && (
                <>
                  <div className="p-3 bg-purple-950/20 border border-purple-500/20 rounded-2xl flex items-center justify-between text-xs">
                    <div className="flex items-center gap-2">
                      <Key className="h-4 w-4 text-purple-400" />
                      <div>
                        <span className="font-bold text-slate-200 block">
                          Tarjeta RFID / Llave Digital Hab.{" "}
                          {selectedRoom.number}
                        </span>
                        <span className="text-[10px] text-slate-400">
                          Integración de cerradura no configurada. Estadía
                          prevista hasta {selectedRoom.checkOutDate} 11:00.
                        </span>
                      </div>
                    </div>
                    <button
                      disabled
                      title="Requiere configurar el proveedor de cerraduras y credenciales del hotel"
                      className="px-2 py-1 bg-slate-900 text-slate-500 text-[10px] font-bold rounded-lg border border-slate-800 cursor-not-allowed"
                    >
                      RFID no configurado
                    </button>
                  </div>
                  ...
                  <div className="p-4 bg-slate-950 border border-slate-850 rounded-2xl text-xs space-y-2">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                      ...
                      <div>
                        ...
                        <span className="font-bold text-slate-300 block mt-0.5">
                          {selectedRoom.guestPassport || "N/A"}
                        </span>
                      </div>
                      <div>
                      ...
                      <div>
                        ...
                        <span
                          className={`font-bold block mt-0.5 ${selectedRoom.securityDepositPaid ? "text-emerald-450" : "text-rose-455"}`}
                        >
                          {selectedRoom.securityDepositPaid
                            ? "Presentada"
                            : "Pendiente"}
                        </span>
                      </div>
                    ...
                    <div className="flex gap-2">
                      <select
                        ...
                        onChange={(e) => {
                          ...
                          if (e.target.value === "Agua Minibar")
                            setExtraPrice(1500);
                          if (e.target.value === "Papas Minibar")
                            setExtraPrice(2500);
                          if (e.target.value === "Room Service Lomo")
                            setExtraPrice(9500);
                          if (e.target.value === "Sesión de Spa")
                            setExtraPrice(25000);
                          if (e.target.value === "Cena Especial")
                            setExtraPrice(18500);
                          if (e.target.value === "Traslado Aeropuerto")
                            setExtraPrice(35000);
                        }}
                        className="flex-1 bg-slate-900 border border-slate-800 rounded-xl px-2 py-1.5 text-xs text-white"
                      >
                        <option value="Agua Minibar">
                          Agua Minibar ($1.500)
                        </option>
                        <option value="Papas Minibar">
                          Papas Minibar ($2.500)
                        </option>
                        <option value="Room Service Lomo">
                          Room Service Lomo ($9.500)
                        </option>
                        <option value="Sesión de Spa">
                          Sesión de Spa ($25.000)
                        </option>
                        <option value="Cena Especial">
                          Cena Especial ($18.500)
                        </option>
                        <option value="Traslado Aeropuerto">
                          Traslado Aeropuerto ($35.000)
                        </option>
                      </select>
                      <button
              ...
              {selectedRoom.status === "cleaning" && (
                <div className="p-4 bg-slate-950 border border-slate-850 rounded-2xl space-y-3">
                  <span className="text-xs font-bold text-purple-400 uppercase block">
                    Asignar Mucama de Limpieza
                  </span>
                  <div className="flex gap-2">
                    <select
                      onChange={(e) => {
                        if (isHousekeeper(e.target.value))
                          handleAssignHousekeeper(e.target.value);
                      }}
                  ...
                  {selectedRoom.housekeeperAssigned && (
                    <span className="text-[10px] text-slate-400 block mt-1">
                      Responsable actual: {selectedRoom.housekeeperAssigned} |
                      Estado: {selectedRoom.cleaningStatus}
                    </span>
                  )}
                </div>
              )}
            ...
            <div className="text-center py-28 text-slate-650 text-xs my-auto">
              ...
              Selecciona una habitación del rack para check-in o facturar
              extras.
            </div>
          )}
        </div>
            ...
            <div className="flex gap-3">
              ...
              <button
                onClick={() => {
                  setFeedback({
                    tone: "success",
                    message: "Comprobante de hotel enviado a impresión.",
                  });
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setSecurityDeposit(e.target.value === "si")
                    }
                    className="w-full bg-slate-950 border border-slate-800 rounded-xl px-2 py-2 text-xs text-white"
                  >
