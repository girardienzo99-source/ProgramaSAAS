...
import { 
  ShieldAlert, ShieldCheck, Search, Users, Activity, CreditCard, 
  Clock, CheckCircle2, XCircle, AlertTriangle, GraduationCap,
  ListFilter, RotateCw, Plus, Heart, UserCheck, DollarSign
} from 'lucide-react';
...
const MEMBER_STATUSES: MemberStatus[] = ['active', 'expired', 'unpaid', 'suspended'];
const PLAN_TYPES: PlanType[] = ['Mensual', 'Trimestral', 'Anual', 'Pase Diario'];
...
const MEMBERS_DATABASE: GymMember[] = [
  { dni: '35123456', name: 'Horacio Quiroga', planName: 'Pase Libre', planType: 'Mensual', expirationDate: '2026-07-28', status: 'active', lastAccess: 'Ayer 18:30 hs', hasMedicalCertificate: true, medicalCertExpiration: '2026-12-15', assignedTrainer: 'Prof. Walter', routineNotes: 'Hipertrofia - Pecho y Bíceps. 4x12.', weeklyFrequencyRate: 3.8 },
  { dni: '40987654', name: 'Clara Vignolo', planName: 'Musculación 3x', planType: 'Trimestral', expirationDate: '2026-07-01', status: 'expired', lastAccess: 'Hace 4 días', hasMedicalCertificate: false, medicalCertExpiration: '2026-05-10', assignedTrainer: 'Prof. Walter', routineNotes: 'Resistencia cardiovascular y tonificación.', weeklyFrequencyRate: 2.1 },
  { dni: '38111222', name: 'Marcos Juárez', planName: 'Crossfit Diarios', planType: 'Mensual', expirationDate: '2026-07-15', status: 'unpaid', lastAccess: 'Hace 2 semanas', hasMedicalCertificate: true, medicalCertExpiration: '2026-08-30', assignedTrainer: 'Prof. Ana', routineNotes: 'Fuerza explosiva, WOD de 20 min.', weeklyFrequencyRate: 1.5 }
];

const INITIAL_CLASSES: GymClass[] = [
  { id: 'c1', name: 'Spinning PRO', time: '08:00', spotsMax: 20, spotsTaken: 18, trainer: 'Prof. Ana', waitingList: [] },
  { id: 'c2', name: 'Yoga Flexibilidad', time: '09:30', spotsMax: 15, spotsTaken: 15, trainer: 'Prof. Walter', waitingList: ['Sofía Martínez'] },
  { id: 'c3', name: 'Zumba Ritmos', time: '19:00', spotsMax: 30, spotsTaken: 12, trainer: 'Prof. Brenda', waitingList: [] }
];
...
export default function GymConsole() {
  ...
  const [scannedMember, setScannedMember] = useState<GymMember | null>(MEMBERS_DATABASE[0]);
  const [accessLog, setAccessLog] = useState<Array<{ name: string; time: string; allowed: boolean; reason?: string }>>([
  ...
  const handleScanAccess = (e: React.FormEvent) => {
    ...
    if (member) {
      ...
      let reason = '';
      if (!isAllowed) {
        reason = member.status === 'expired' ? 'Cuota Expirada' : 'Falta de Pago';
      ...
      setAccessLog(prev => [
        { name: member.name, time: new Date().toLocaleTimeString().slice(0, 5) + ' hs', allowed: isAllowed, reason },
        ...prev
      ]);
    } else {
      setScannedMember({
      ...
      setAccessLog(prev => [
        { name: `DNI ${searchDni}`, time: new Date().toLocaleTimeString().slice(0, 5) + ' hs', allowed: false, reason: 'No Registrado' },
        ...prev
      ]);
    }
    setSearchDni('');
  ...
  const handleBookClass = (classId: string) => {
    setGymClasses(prev => prev.map(c => {
      if (c.id === classId) {
        ...
        if (isFull) {
          setFeedback({ type: 'success', message: `Clase completa. Agregado a lista de espera para ${c.name}.` });
          return { ...c, waitingList: [...c.waitingList, scannedMember?.name || 'Invitado'] };
        } else {
          setFeedback({ type: 'success', message: `Reserva confirmada para la clase de ${c.name}.` });
  ...
  const handlePayFee = (memberDni: string) => {
    ...
    setFeedback({ type: 'success', message: 'Pago de membresía procesado. Plan renovado por 30 días.' });
  };
  ...
  const handleRenewMedicalCert = (memberDni: string) => {
    setMembers(prev => prev.map(m => {
      if (m.dni === memberDni) {
        const updated: GymMember = {
          ...
          medicalCertExpiration: new Date(Date.now() + 365 * 24 * 60 * 60 * 1000).toISOString().split('T')[0]
        };
        if (scannedMember && scannedMember.dni === memberDni) {
          setScannedMember(updated);
        }
    ...
    setFeedback({ type: 'success', message: 'Apto médico renovado por 1 año.' });
  };
  ...
  const handleAssignRoutine = (e: React.FormEvent) => {
    ...
    setFeedback({ type: 'success', message: `Rutina de ${routineMuscle} asignada con éxito a ${scannedMember.name}.` });
  };

  // Filtrado de socios
  const filteredMembers = useMemo(() => members.filter(m => {
    const matchSearch = m.name.toLowerCase().includes(searchTerm.toLowerCase()) || m.dni.includes(searchTerm);
    ...
    <div className="w-full max-w-7xl mx-auto px-4 py-6 text-slate-100 flex flex-col gap-6 font-sans">
      ...
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center bg-slate-900/60 p-5 rounded-2xl border border-slate-800 gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-black text-white">Consola de Control de Accesos & Clases</h1>
            <p className="text-slate-400 text-xs mt-0.5">Validación de molinetes en tiempo real, aptos médicos y listas de espera de actividades.</p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
        <div className="bg-slate-900/50 p-4 rounded-xl border border-slate-850">
          <span className="text-[10px] text-slate-500 font-bold uppercase block">Socios Activos</span>
          <span className="text-xl font-black text-white mt-1 block">
            {members.filter(m => m.status === 'active').length}
          </span>
        </div>
        <div className="bg-slate-900/50 p-4 rounded-xl border border-slate-850">
          <span className="text-[10px] text-slate-500 font-bold uppercase block">Accesos de Hoy</span>
          <span className="text-xl font-black text-cyan-400 mt-1 block">{accessLog.length}</span>
        </div>
        <div className="bg-slate-900/50 p-4 rounded-xl border border-slate-850">
          <span className="text-[10px] text-slate-500 font-bold uppercase block">Morosos / Vencidos</span>
          <span className="text-xl font-black text-rose-500 mt-1 block">
            {members.filter(m => m.status === 'unpaid' || m.status === 'expired').length}
          </span>
        </div>
        <div className="bg-slate-900/50 p-4 rounded-xl border border-slate-850">
          <span className="text-[10px] text-slate-500 font-bold uppercase block">Faltan Aptos Médicos</span>
          <span className="text-xl font-black text-amber-500 mt-1 block">
            {members.filter(m => !m.hasMedicalCertificate).length}
          </span>
        </div>
        <div className="bg-slate-900/50 p-4 rounded-xl border border-slate-850">
          <span className="text-[10px] text-slate-500 font-bold uppercase block">Asistencia Clases</span>
      ...
      {members.some(m => !m.hasMedicalCertificate) && (
        <div className="p-4 bg-amber-550/10 border border-amber-500/25 rounded-2xl flex items-center justify-between">
          <div className="flex items-center gap-2.5 text-xs text-amber-350 font-semibold">
            ...
            <span>Alerta Médica: Existen socios activos sin el apto médico vigente presentado en secretaría.</span>
          </div>
        </div>
      )}
      ...
      <div className="bg-slate-900/40 p-5 rounded-2xl border border-slate-850 grid grid-cols-1 sm:grid-cols-3 gap-3">
        <div className="space-y-1">
          <label className="text-[9px] text-slate-500 font-bold uppercase block">Buscar Socio por DNI o Nombre</label>
        ...
        <div className="space-y-1">
          <label className="text-[9px] text-slate-500 font-bold uppercase block">Estado Socio</label>
          <select
            value={statusFilter}
            onChange={e => {
        ...
        <div className="space-y-1">
          <label className="text-[9px] text-slate-500 font-bold uppercase block">Frecuencia / Tipo Plan</label>
          <select
            value={planFilter}
            onChange={e => {
              const value = e.target.value;
        ...
        <div className="lg:col-span-1 bg-slate-900/40 p-5 rounded-3xl border border-slate-850 flex flex-col justify-between min-h-[440px]">
          <div className="space-y-4">
            ...
            <form onSubmit={handleScanAccess} className="space-y-2">
              <label className="text-[10px] text-slate-500 font-bold uppercase block">Ingresar DNI Socio</label>
            ...
            <div className="bg-slate-950 p-3.5 rounded-xl border border-slate-900 flex items-center justify-between mt-2">
              <span className="text-[9px] text-slate-500 font-bold uppercase tracking-wider block">Estado del Molinete IoT</span>
              {scannedMember && scannedMember.status === 'active' && scannedMember.hasMedicalCertificate ? (
                <div className="flex items-center gap-1.5">
                  <span className="h-2.5 w-2.5 rounded-full bg-emerald-500 animate-ping"></span>
                  <span className="text-[10px] text-emerald-450 font-bold uppercase tracking-widest bg-emerald-500/10 px-2 py-0.5 rounded border border-emerald-500/20">MOLINETE ABIERTO</span>
                </div>
              ) : (
                <div className="flex items-center gap-1.5">
                  <span className="h-2.5 w-2.5 rounded-full bg-rose-500"></span>
                  <span className="text-[10px] text-rose-450 font-bold uppercase tracking-widest bg-rose-500/10 px-2 py-0.5 rounded border border-rose-500/20">MOLINETE BLOQUEADO</span>
                </div>
              )}
            </div>
          </div>
          ...
          {scannedMember ? (
            <div className={`p-4 rounded-2xl border text-center space-y-3 mt-4 ${
              scannedMember.status === 'active' && scannedMember.hasMedicalCertificate
              ...
              <div>
                <h4 className="font-extrabold text-sm text-white">{scannedMember.name}</h4>
                <span className="text-[10px] text-slate-400 block mt-0.5">Plan: {scannedMember.planName} ({scannedMember.planType})</span>
                <span className="text-[10px] text-slate-500 block">Vto: {scannedMember.expirationDate}</span>
                <span className="text-[10px] text-cyan-400 block font-bold mt-1">Frecuencia: {scannedMember.weeklyFrequencyRate} visitas/semana</span>
                
                {!scannedMember.hasMedicalCertificate ? (
                  <span className="text-[9px] text-rose-450 font-bold uppercase block mt-1.5 animate-pulse">✗ FALTA APTO MÉDICO (Vto: {scannedMember.medicalCertExpiration})</span>
                ) : (
                  <span className="text-[9px] text-emerald-450 font-bold uppercase block mt-1.5">✓ Apto Médico Vigente</span>
                )}
              </div>
            ...
            <div className="bg-slate-900/40 p-5 rounded-3xl border border-slate-850 space-y-3 animate-fade-in">
              <div className="flex justify-between items-center border-b border-slate-900 pb-2">
                <span className="text-xs font-bold text-rose-450 uppercase tracking-wider block flex items-center gap-1.5">
                  ...
                  Planificación & Rutina Activa (Entrenador: {scannedMember.assignedTrainer})
                </span>
              </div>
              <div className="p-3 bg-slate-950 border border-slate-900 rounded-xl">
                <span className="text-[9px] text-slate-500 font-bold uppercase block">Rutina Diaria</span>
                <p className="text-xs text-slate-300 italic mt-1">"{scannedMember.routineNotes}"</p>
              </div>

              {/* Constructor de Rutinas Integrado */}
              <form onSubmit={handleAssignRoutine} className="p-4 bg-slate-950/60 border border-slate-900 rounded-xl space-y-3">
                <span className="text-[10px] text-slate-500 font-bold uppercase block">Modificar Rutina de Entrenamiento</span>
                
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5 text-xs">
                  <div className="space-y-1">
                    <label className="text-[9px] text-slate-550 block font-bold uppercase">Grupo Muscular</label>
                    <select
                      value={routineMuscle}
                  ...
                  <div className="space-y-1">
                    <label className="text-[9px] text-slate-550 block font-bold uppercase">Ejercicio</label>
                    <input
                      type="text"
                  ...
                  <div className="space-y-1">
                    <label className="text-[9px] text-slate-550 block font-bold uppercase">Series</label>
                    <input
                      type="number"
                      value={routineSets}
                  ...
                  <div className="space-y-1">
                    <label className="text-[9px] text-slate-550 block font-bold uppercase">Repeticiones</label>
                    <input
                      type="number"
                      value={routineReps}
          ...
          <div className="bg-slate-900/40 p-6 rounded-3xl border border-slate-850 space-y-4">
            ...
            <div className="space-y-3">
              {gymClasses.map(c => {
                ...
                return (
                  <div key={c.id} className="p-4 bg-slate-950 border border-slate-850 rounded-2xl flex flex-col md:flex-row justify-between gap-4 items-start md:items-center">
                    <div className="space-y-1">
                      <span className="font-extrabold text-xs text-white block">{c.name}</span>
                      <span className="text-[10px] text-slate-500 block font-semibold">Horario: {c.time} hs | Instructor: {c.trainer}</span>
                    ...
                    <div className="flex gap-4 items-center w-full md:w-auto border-t md:border-t-0 border-slate-900 pt-3 md:pt-0 justify-between">
                      <div className="text-left md:text-right shrink-0">
                        <span className="text-[9px] text-slate-550 uppercase block font-bold">Cupos</span>
                        <span className="font-black text-cyan-400 text-xs">{c.spotsTaken} / {c.spotsMax}</span>
                      </div>

                      <button
                        onClick={() => handleBookClass(c.id)}
                        className={`px-3 py-1.5 rounded-lg text-[9px] font-black uppercase ${
                          full ? 'bg-amber-600 hover:bg-amber-500 text-slate-950 animate-pulse' : 'bg-cyan-600 hover:bg-cyan-500 text-slate-950'
                        }`}
                      >
