...
const MOCK_PRODUCTS = [
  {
    ...
    category: "Tops",
    sizeChart: { S: 92, M: 100, L: 108, XL: 116 },
  },
  {
    id: "p2",
    name: "Pantalón Jean Slim Fit",
    category: "Bottoms",
    sizeChart: { S: 80, M: 88, L: 96, XL: 104 },
  },
  {
    id: "p3",
    name: "Saco Sastrero Lino",
    category: "Tops",
    sizeChart: { S: 96, M: 104, L: 112, XL: 120 },
  },
];
...
export default function FittingRoomConsole() {
  ...
  const getZoneFit = (zoneName: "chest" | "waist" | "hip") => {
    ...
    if (zoneName === "chest") {
      targetGarmentValue =
        selectedProduct.category === "Tops" ? baseValue : baseValue * 1.15;
      userMeasure = chest;
    } else if (zoneName === "waist") {
      targetGarmentValue =
        selectedProduct.category === "Bottoms" ? baseValue : baseValue * 0.84;
      userMeasure = waist;
    } else if (zoneName === "hip") {
      targetGarmentValue =
        selectedProduct.category === "Bottoms"
          ? baseValue * 1.1
          : baseValue * 0.95;
      userMeasure = hip;
    }
  ...
  return (
    <div className="w-full max-w-6xl mx-auto px-4 py-8 text-slate-100 flex flex-col gap-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-slate-900/60 p-6 rounded-2xl border border-slate-800 backdrop-blur-md">
        <div>
          ...
          <p className="mt-2 text-slate-400 text-sm">
            Recomendador inteligente de talles para e-commerce. Analizá el calce
            anatómico exacto y reducí tasas de devolución.
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-slate-900/40 p-6 rounded-2xl border border-slate-800 flex flex-col items-center justify-between">
            ...
            {calculatedSize && (
              <div className="absolute inset-0 pointer-events-none text-[8px] font-bold font-mono">
                <div className="absolute top-[50px] right-[-15px] bg-slate-950/95 border border-slate-800/80 px-1.5 py-0.5 rounded text-slate-300">
                  Pecho:{" "}
                  <span className={chestFit.textClass}>{chestFit.label}</span>
                </div>
                <div className="absolute top-[85px] left-[-20px] bg-slate-950/95 border border-slate-800/80 px-1.5 py-0.5 rounded text-slate-300">
                  Cintura:{" "}
                  <span className={waistFit.textClass}>{waistFit.label}</span>
                </div>
                <div className="absolute top-[120px] right-[-15px] bg-slate-950/95 border border-slate-800/80 px-1.5 py-0.5 rounded text-slate-300">
                  Cadera:{" "}
                  <span className={hipFit.textClass}>{hipFit.label}</span>
                </div>
              </div>
            )}
          </div>
          ...
          <div className="text-center text-[10px] text-slate-500 mt-4 leading-relaxed">
            El mapa de calor representa los puntos de tensión del hilado según
            la tabla de elasticidad del producto.
          </div>
        </div>
        ...
        <div className="bg-slate-900/40 p-6 rounded-2xl border border-slate-800 flex flex-col justify-between">
          <div>
            ...
            {calculatedSize ? (
              <div className="space-y-6">
                ...
                <div className="space-y-3.5 text-xs">
                  ...
                  <div className="space-y-2 py-1.5">
                    <div>
                      <div className="flex justify-between text-[9px] text-slate-450 mb-0.5">
                        ...
                        <span
                          className={`${chestFit.textClass} font-mono font-bold`}
                        >
                          {chestFit.ratio}% ({chestFit.label})
                        </span>
                      </div>
                      <div className="w-full bg-slate-950 rounded-full h-1">
                        <div
                          ...
                          style={{
                            width: `${Math.min(100, chestFit.ratio)}%`,
                            backgroundColor: chestFit.color,
                          }}
                        />
                      </div>
                    </div>
                    ...
                    <div>
                      <div className="flex justify-between text-[9px] text-slate-450 mb-0.5">
                        ...
                        <span
                          className={`${waistFit.textClass} font-mono font-bold`}
                        >
                          {waistFit.ratio}% ({waistFit.label})
                        </span>
                      </div>
                      <div className="w-full bg-slate-950 rounded-full h-1">
                        <div
                          className="h-full rounded-full transition-all duration-500"
                          style={{
                            width: `${Math.min(100, waistFit.ratio)}%`,
                            backgroundColor: waistFit.color,
                          }}
                        />
                      </div>
                    </div>
                    ...
                    <div>
                      <div className="flex justify-between text-[9px] text-slate-450 mb-0.5">
                        ...
                        <span
                          className={`${hipFit.textClass} font-mono font-bold`}
                        >
                          {hipFit.ratio}% ({hipFit.label})
                        </span>
                      </div>
                      <div className="w-full bg-slate-950 rounded-full h-1">
                        <div
                          className="h-full rounded-full transition-all duration-500"
                          style={{
                            width: `${Math.min(100, hipFit.ratio)}%`,
                            backgroundColor: hipFit.color,
                          }}
                        />
                      </div>
                    </div>
                  ...
                  <div className="border-t border-slate-850 pt-2.5 space-y-1">
                    ...
                    <p className="text-[10px] text-slate-350 leading-relaxed font-sans">
                      {chestFit.label === "Ajustado" ||
                      waistFit.label === "Ajustado" ||
                      hipFit.label === "Ajustado" ? (
                        <span className="text-amber-500 font-semibold">
                          ⚠️ Ciertas zonas quedarán ceñidas al cuerpo. Si
                          preferís un estilo más relajado, te sugerimos un talle
                          extra.
                        </span>
                      ) : chestFit.label === "Holgado" &&
                        waistFit.label === "Holgado" ? (
                        <span className="text-cyan-400 font-semibold">
                          ✨ El calce tendrá un estilo Oversized relajado, ideal
                          para siluetas street-wear.
                        </span>
                      ) : (
                        <span className="text-emerald-450 font-semibold">
                          ✅ El talle seleccionado es el óptimo para tu
                          estructura corporal según el patrón de costura.
                        </span>
                      )}
                    </p>
                  </div>
                ...
                <div className="p-3 bg-emerald-950/15 border border-emerald-900/30 rounded-xl flex gap-2">
                  ...
                  <p className="text-[10px] text-slate-400 leading-normal">
                    Este talle cuenta con garantía de primer cambio gratis
                    directo en e-commerce.
                  </p>
                </div>
              </div>
            ) : (
              <div className="text-center py-20 text-slate-650 text-xs">
                ...
                Ingresá las medidas del cliente y haz clic en "Estimar Talle
                Ideal" para ver el diagnóstico.
              </div>
            )}
          </div>
          ...
          {calculatedSize && (
            <button
              onClick={() =>
                alert(
                  "Imprimiendo prescripción de talle con código de barra para mostrador...",
                )
              }
      ...
      {/* FASE 3: Tabla Comparativa de Talles de Fábrica & Compra Rápida */}
      {calculatedSize && (
        <div className="bg-slate-900/40 p-6 rounded-2xl border border-slate-800 space-y-4 text-xs text-left">
          {/* FASE 4: Matriz Multidimensional de Stock por Talle y Color */}
          <div className="p-3 bg-indigo-950/20 border border-indigo-500/20 rounded-2xl flex items-center justify-between text-xs">
            <div className="flex items-center gap-2">
              <Layers className="h-4 w-4 text-indigo-400" />
              <div>
                <span className="font-bold text-slate-200 block">
                  Matriz de Stock por Color & Talle (SKU Matrix)
                </span>
                <span className="text-[10px] text-slate-400">
                  Negro: S(4), M(12), L(8), XL(2) | Blanco: S(2), M(6), L(3),
                  XL(0) | Azul: S(5), M(9), L(4), XL(1)
                </span>
              </div>
            </div>
            <span className="px-2 py-1 bg-indigo-500/10 text-indigo-400 text-[10px] font-bold rounded-lg border border-indigo-500/20 uppercase">
              Referencia no sincronizada
            </span>
          </div>

          <div className="flex justify-between items-center border-b border-slate-850 pb-3">
            <h3 className="font-extrabold text-sm text-white uppercase tracking-wider flex items-center gap-1.5">
              <Scale className="h-4.5 w-4.5 text-cyan-455 animate-pulse" />
              Equivalencias de Talles de Fábrica & Ficha de Medidas
            </h3>
            <span className="text-[10px] text-slate-500">
              Resaltando: Talle {calculatedSize}
            </span>
          </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-slate-350">
              <thead>
                <tr className="border-b border-slate-850 text-slate-500 text-[10px] uppercase font-bold text-center">
                  <th className="py-2 text-left">Talle</th>
                  <th>Busto / Pecho (cm)</th>
                  <th>Cintura (cm)</th>
                  <th>Cadera (cm)</th>
                  <th>Equivalencia AR</th>
                  <th>Equivalencia US/EU</th>
                  <th>Estado Recomendación</th>
                </tr>
              </thead>
              <tbody>
                {["S", "M", "L", "XL"].map((size) => {
                  const isSugerido = calculatedSize === size;

                  const chart = selectedProduct.sizeChart as Record<
                    string,
                    number
                  >;
                  const chestVal =
                    size === "S"
                      ...
                      : size === "M"
                        ...
                        : size === "L"
                          ? chart.L
                          : chart.XL;
                  const waistVal = Math.round(chestVal * 0.84);
                  const hipVal = Math.round(chestVal * 0.96);
                  ...
                  return (
                    <tr
                      key={size}
                      className={`border-b border-slate-900 text-center transition-all ${
                        isSugerido
                          ? "bg-cyan-500/10 text-cyan-400 font-extrabold border-l-4 border-l-cyan-500"
                          : "hover:bg-slate-950/20"
                      }`}
                    >
                      <td className="py-3 px-2 text-left font-bold">{size}</td>
                      <td>{chestVal} cm</td>
                      <td>{waistVal} cm</td>
                      <td>{hipVal} cm</td>
                      <td>{eqAr}</td>
                      ...
                      <td>
                        {isSugerido ? (
                          <span className="inline-flex px-2 py-0.5 bg-cyan-500/25 border border-cyan-400 rounded text-[9px] font-bold uppercase tracking-wider animate-pulse">
                            Sugerido IA
                          </span>
                        ) : (
          ...
          <div className="flex flex-col sm:flex-row justify-between items-center gap-4 bg-slate-950/50 p-4 rounded-xl border border-slate-850">
            <div>
              <span className="text-[10px] text-slate-500 uppercase font-bold block">
                Acción de Asistente de Ventas
              </span>
              <p className="text-[10px] text-slate-350 mt-0.5">
                Añadir al probador/carrito de compra el talle sugerido por IA (
                {calculatedSize}) de {selectedProduct.name}.
              </p>
            </div>
            <button
              onClick={() =>
                alert(
                  `Selección preparada para mostrador: ${selectedProduct.name} - Talle ${calculatedSize}. Confirmá stock antes de vender.`,
                )
              }
              className="px-4 py-2 bg-gradient-to-r from-cyan-500 to-indigo-500 hover:from-cyan-600 hover:to-indigo-600 text-white font-bold rounded-xl text-xs uppercase shadow-lg shadow-indigo-500/20"
            >
              Preparar talle {calculatedSize}
            </button>
          </div>
        </div>
      )}
