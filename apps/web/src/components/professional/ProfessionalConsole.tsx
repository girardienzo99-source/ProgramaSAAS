...
type ProjectStatus =
  "planning" | "development" | "testing" | "delivered" | "support";
...
const INITIAL_PROJECTS: ProfessionalProject[] = [
  ...
  },
  {
    ...
    milestones: [
      ...
      {
        name: "Presentación Informe Ejecutivo",
        completed: true,
        dueDate: "2026-07-05",
      },
    ],
...
export default function ProfessionalConsole() {
  const [projects, setProjects] =
    useState<ProfessionalProject[]>(INITIAL_PROJECTS);
  const [selectedProject, setSelectedProject] =
    useState<ProfessionalProject | null>(INITIAL_PROJECTS[0]);
  ...
  const handleCreateProject = (e: React.FormEvent) => {
    ...
    if (!projectName || !clientName || hourlyRate <= 0 || estimatedHours <= 0) {
      setFeedback({
        tone: "error",
        message:
          "Completá proyecto, cliente, tarifa y horas con valores válidos.",
      });
      return;
    }
    ...
    const newProj: ProfessionalProject = {
      ...
      milestones: [
        {
          name: "Kick-off inicial de proyecto",
          completed: false,
    ...
    setFeedback({
      tone: "success",
      message: `Proyecto ${newProj.projectName} creado y listo para cargar horas.`,
    });
  ...
  const handleLogHours = (e: React.FormEvent) => {
    ...
    if (!selectedProject || hoursToAdd <= 0) {
      setFeedback({
        tone: "error",
        message: "Ingresá una cantidad de horas mayor a cero.",
      });
      return;
    }
    ...
    setFeedback({
      tone:
        updated.actualHours > updated.estimatedHours ? "warning" : "success",
  ...
  const handleToggleMilestone = (idx: number) => {
    ...
    const milestones = selectedProject.milestones.map(
    ...
    if (allCompleted) {
      setFeedback({
        tone: "success",
        message: `¡Excelente! Todos los hitos del proyecto "${selectedProject.projectName}" se han completado con éxito.`,
      });
    } else {
      setFeedback({
        tone: "success",
        message: `Hito "${milestones[idx].name}" actualizado.`,
      });
    }
  };
  ...
  const handlePostponeMilestone = (idx: number) => {
    ...
    setFeedback({
      tone: "warning",
      message: `Fecha límite de "${selectedProject.milestones[idx].name}" postergada por 7 días.`,
    });
  };
  ...
  const handleUpdateStatus = (status: ProjectStatus) => {
    ...
    setFeedback({
      tone: "success",
      message: `Proyecto actualizado a ${PROJECT_STATUS_LABELS[status]}.`,
    });
  };
  ...
  const handleGenerateInvoice = (proj: ProfessionalProject) => {
    ...
    setFeedback({
      tone: "success",
      message: "Factura electrónica emitida ante AFIP con éxito.",
    });
  };
  ...
  const metrics = useMemo(() => {
    const totalHourlyRate = projects.reduce(
      (acc, curr) => acc + curr.hourlyRate,
      0,
    );
    const averageRate =
      projects.length > 0 ? Math.round(totalHourlyRate / projects.length) : 0;

    return {
      activeProjects: projects.length,
      hoursLoggedToday: projects.reduce(
        (acc, curr) => acc + curr.hoursLoggedToday,
        0,
      ),
      invoicedIncome: projects
        .filter((p) => p.isInvoiced)
        .reduce((acc, curr) => acc + curr.budgetAmount, 0),
      overBudgetProjects: projects.filter(
        (p) => p.actualHours > p.estimatedHours,
      ).length,
      averageRate,
    };
  }, [projects]);
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-black text-white">
              Consola de Horas, Hitos & Proyectos
            </h1>
            <p className="text-slate-400 text-xs mt-0.5">
              Control de horas de consultores, desvíos presupuestarios e hitos
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
        ...
        <div className="bg-slate-900/50 p-4 rounded-xl border border-slate-850">
          ...
          <span className="text-xl font-black text-indigo-400 mt-1 block">
            ${metrics.averageRate.toLocaleString()}
          </span>
        </div>
      </div>
      ...
      {projects.some((p) => p.actualHours > p.estimatedHours) && (
        <div className="p-4 bg-rose-550/10 border border-rose-500/25 rounded-2xl flex items-center justify-between">
          <div className="flex items-center gap-2.5 text-xs text-rose-350 font-semibold">
            ...
            <span>
              Alerta de Consultoría: Existen proyectos activos que han excedido
              las horas presupuestadas iniciales.
            </span>
          </div>
        </div>
      )}
          ...
          {selectedProject ? (
            <>
              <div className="flex justify-between items-start pb-4 border-b border-slate-850">
                <div>
                  ...
                  <span className="text-[10px] text-slate-500 block mt-0.5">
                    Cliente: {selectedProject.clientName} | Responsable:{" "}
                    {selectedProject.owner} | Tarifa: $
                    {selectedProject.hourlyRate}/h
                  </span>
                </div>
              ...
              <div className="p-4 bg-slate-950 border border-slate-850 rounded-2xl space-y-4">
                <div className="flex justify-between items-center">
                  <span className="text-[10px] text-slate-550 font-bold uppercase block">
                    Carga de Horas del Proyecto
                  </span>
                  <span className="text-[10px] text-slate-500">
                    Consumidas: {selectedProject.actualHours} hs / Estimadas:{" "}
                    {selectedProject.estimatedHours} hs
                  </span>
                </div>
                ...
                <div className="w-full bg-slate-900 rounded-full h-3.5 overflow-hidden border border-slate-850">
                  <div
                    ...
                    style={{
                      width: `${Math.min(100, (selectedProject.actualHours / selectedProject.estimatedHours) * 100)}%`,
                    }}
                  />
                </div>
                ...
                {selectedProject.actualHours >
                  ...
                  <span className="text-[9px] text-rose-500 font-bold uppercase block">
                    ⚠ Desvío detectado: Se excedió el presupuesto original por{" "}
                    {selectedProject.actualHours -
                      selectedProject.estimatedHours}{" "}
                    hs.
                  </span>
                ...
                <form
                  ...
                  className="grid grid-cols-1 md:grid-cols-3 gap-3 border-t border-slate-900 pt-3.5"
                >
                  <input
                    type="number"
                    required
                  ...
                  <button
                    type="submit"
                    className="py-1.5 bg-indigo-600 hover:bg-indigo-500 text-white font-bold rounded-xl text-xs"
                  >
                    Cargar Horas
                  </button>
                </form>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <span className="font-bold text-slate-200 block">
                      Rendición de Gastos Reembolsables & Viáticos
                    </span>
                    <span className="text-[10px] text-slate-400">
                      No hay comprobantes cargados. Las rendiciones deben
                      registrar fecha, categoría, importe, archivo y aprobación.
                    </span>
                  </div>
                </div>
              ...
              <div className="p-4 bg-slate-950 border border-slate-850 rounded-2xl space-y-4 text-xs">
                <div className="flex justify-between items-center">
                  <span className="text-[10px] text-slate-550 font-bold uppercase block">
                    Simulador de Margen y Desvío Financiero
                  </span>
                  <span
                    className={`px-2 py-0.5 rounded text-[8px] font-bold uppercase ${
                      selectedProject.actualHours * selectedProject.hourlyRate >
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 text-center">
                  <div className="p-3 bg-slate-900 border border-slate-800 rounded-xl">
                    ...
                    <span className="font-mono font-black text-slate-200 mt-1 block">
                  ...
                  <div className="p-3 bg-slate-900 border border-slate-800 rounded-xl">
                    <span className="text-[9px] text-slate-500 font-bold block uppercase">
                      Margen Restante Proyectado
                    </span>
                    <span
                      className={`font-mono font-black mt-1 block ${
                        selectedProject.budgetAmount -
                          (selectedProject.actualHours + projectedExtraHours) *
                ...
                <div className="space-y-1.5 pt-1 border-t border-slate-900">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setProjectedExtraHours(Number(e.target.value))
                    }
                    className="w-full h-1 bg-slate-900 rounded-lg appearance-none cursor-pointer accent-indigo-500"
                  />
                  <p className="text-[9px] text-slate-500">
                    Arrastrá el control para prever si futuras horas extras
                    causarán desvío en el presupuesto contractual.
                  </p>
                </div>
              </div>
                  ...
                  {selectedProject.milestones.map((m, idx) => (
                    ...
                    >
                      <div className="flex items-center gap-2">
                        ...
                        />
                        <span
                          className={`font-semibold ${m.completed ? "line-through text-slate-500" : "text-slate-200"}`}
                        >
                          {m.name}
                        </span>
                      </div>
            ...
            <div className="text-center py-28 text-slate-650 text-xs italic">
              Selecciona un proyecto para cargar horas de consultores y
              gestionar hitos.
            </div>
          )}
        </div>
            ...
            <div className="flex gap-3">
              ...
              <button
                onClick={() => {
                  setFeedback({
                    tone: "success",
                    message: "Reporte comercial enviado a impresión.",
                  });
            ...
            <form onSubmit={handleCreateProject} className="space-y-4">
              ...
              <div>
                ...
                <select
                  value={newOwner}
                  onChange={(e) => {
                    if (isProjectOwner(e.target.value))
