...
type GarmentType =
  ...
  | "Alfombra";
type StainType =
  "Ninguna" | "Grasa/Aceite" | "Tinta" | "Café/Vino" | "Óxido" | "Barro";
type WashingCycle =
  "Normal" | "Ecológico (Eco)" | "Delicado/Seda" | "DryClean (Seco)";
type ServiceType =
  "Lavado" | "Secado" | "Planchado" | "Tintorería" | "Costura" | "Express";
type LaundryStatus =
  | "received"
  | "classified"
  | "washing"
...
const WASHING_CYCLES: WashingCycle[] = [
  "Normal",
  "Ecológico (Eco)",
  "Delicado/Seda",
  "DryClean (Seco)",
...
const LAUNDRY_STATUS_STEPS: Array<{ label: string; status: LaundryStatus }> =
  Object.entries(LAUNDRY_STATUS_LABELS).map(([status, label]) => ({
    status: status as LaundryStatus,
    label,
  }));
...
const INITIAL_LAUNDRY: LaundryItem[] = [
  {
    id: "l-1001",
    clientName: "Horacio Quiroga",
    clientPhone: "11-5841-2532",
    garmentType: "Saco",
    color: "Gris Melange",
    material: "Lana Merino",
    qty: 1,
    stainNotes: "Mancha de grasa en solapa izquierda.",
    stainType: "Grasa/Aceite",
    washingCycle: "DryClean (Seco)",
    serviceType: "Tintorería",
    status: "washing",
  ...
  {
    id: "l-1002",
    clientName: "Clara Vignolo",
    clientPhone: "11-9988-7733",
    garmentType: "Vestido",
    color: "Rojo Carmín",
    material: "Seda",
    qty: 1,
    stainNotes: "Sin manchas. Lavado ecológico.",
    stainType: "Ninguna",
    washingCycle: "Delicado/Seda",
    serviceType: "Tintorería",
    status: "ironing",
  ...
  },
  {
    ...
    clientPhone: "11-5544-3322",
    garmentType: "Camisa",
    color: "Blanco",
    material: "Algodón",
    qty: 3,
    stainNotes: "Manchas de café en puño derecho.",
    stainType: "Café/Vino",
    washingCycle: "Normal",
...
export default function LaundryConsole() {
  const [laundryItems, setLaundryItems] =
    useState<LaundryItem[]>(INITIAL_LAUNDRY);
  const [selectedItem, setSelectedItem] = useState<LaundryItem | null>(
    INITIAL_LAUNDRY[0],
  );
  ...
  const STAIN_RECIPES: Record<StainType, string> = {
    ...
    "Grasa/Aceite":
      "Tratamiento: Aplicar solvente orgánico pre-lavado en seco. Centrifugado a temperatura templada.",
    Tinta:
      "Tratamiento: Tratar mancha con alcohol isopropílico antes de lavado. No planchar hasta remover.",
    "Café/Vino":
      "Tratamiento: Limpieza con solución ácida diluida y agua fría. Enjuague abundante.",
    Óxido:
      "Tratamiento: Tratar con ácido oxálico. Lavar por separado para evitar transferencia.",
    Barro:
      "Tratamiento: Dejar secar, cepillar el exceso. Lavado con detergente enzimático estándar.",
  };
  ...
  const [newGarment, setNewGarment] =
    useState<LaundryItem["garmentType"]>("Camisa");
  ...
  const [newStainType, setNewStainType] =
    useState<LaundryItem["stainType"]>("Ninguna");
  const [newCycle, setNewCycle] =
    useState<LaundryItem["washingCycle"]>("Normal");
  const [newService, setNewService] =
    useState<LaundryItem["serviceType"]>("Lavado");
  ...
  const handleRegisterGarment = (e: React.FormEvent) => {
    ...
    if (!clientName || qty <= 0 || price <= 0) {
      setFeedback({
        tone: "error",
        message: "Completá cliente, cantidad y precio con valores válidos.",
      });
      return;
    }
    ...
    setFeedback({
      tone: "success",
      message: "Prendas ingresadas y ticket de lavado emitido.",
    });
  };
  ...
  const handleUpdateStatus = (id: string, status: LaundryStatus) => {
    ...
    setFeedback({
      tone: "success",
      message: `Estado actualizado a ${LAUNDRY_STATUS_LABELS[status]}.`,
    });
  };
  ...
  const handlePayTicket = () => {
    ...
    setFeedback({
      tone: "success",
      message: "Pago de ticket de tintorería registrado con éxito.",
    });
  };
  ...
  const handleStartMachineCycle = (machineId: string) => {
    ...
    setFeedback({
      tone: "success",
      message: `Ciclo de lavado/secado iniciado en máquina para el ticket ${selectedItem.id}.`,
    });
  };
  ...
  const metrics = useMemo(
    () => ({
      activeCount: laundryItems.filter((item) => item.status !== "delivered")
        .length,
      inProcessCount: laundryItems.filter((item) =>
        ["washing", "drying", "ironing"].includes(item.status),
      ).length,
      readyCount: laundryItems.filter((item) => item.status === "ready").length,
      dueTodayCount: laundryItems.filter(
        (item) =>
          item.deliveryDate <= new Date().toISOString().split("T")[0] &&
          item.status !== "delivered",
      ).length,
      paidRevenue: laundryItems
  ...
  const filteredLaundryItems = useMemo(
    () =>
      laundryItems.filter((item) => {
        ...
        const matchGarment =
          garmentFilter === "all" || item.garmentType === garmentFilter;
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-black text-white">
              Recepción de Prendas & Control de Taller
            </h1>
            <p className="text-slate-400 text-xs mt-0.5">
              Control de lavado, secado, planchado y rotulado con especificación
              de manchas.
            </p>
          </div>
        </div>
      ...
      {laundryItems.some((item) => !item.isPaid && item.status === "ready") && (
        <div className="p-4 bg-amber-550/10 border border-amber-500/25 rounded-2xl flex items-center justify-between">
          <div className="flex items-center gap-2.5 text-xs text-amber-300 font-semibold">
            ...
            <span>
              Alerta de Caja: Existen prendas en estado "Listo para Entrega" con
              saldos de pago pendientes de cobro.
            </span>
          </div>
        </div>
      )}
      ...
      <div className="bg-slate-900/40 p-5 rounded-2xl border border-slate-850 grid grid-cols-1 sm:grid-cols-3 gap-3">
        ...
        <div className="space-y-1">
          ...
          <select
            value={garmentFilter}
            onChange={(e) => {
              if (isGarmentFilter(e.target.value))
                setGarmentFilter(e.target.value);
            }}
        ...
        <div className="space-y-1">
          ...
          <select
            value={serviceFilter}
            onChange={(e) => {
              if (isServiceFilter(e.target.value))
              ...
              return (
                ...
                >
                  ...
                  <span className="text-[10px] text-slate-400 block mt-1">
                    {item.garmentType} ({item.color}) | Cant: {item.qty}
                  </span>
        ...
        <div className="lg:col-span-2 bg-slate-900/40 p-6 rounded-3xl border border-slate-850 flex flex-col justify-between min-h-[480px]">
          {selectedItem ? (
              ...
              <div className="flex justify-between items-start pb-4 border-b border-slate-850">
                <div>
                  <h4 className="font-black text-base text-white">
                    {selectedItem.garmentType} ({selectedItem.material})
                  </h4>
                  <span className="text-[10px] text-slate-500 block mt-0.5">
                    Cliente: {selectedItem.clientName} ($
                    {selectedItem.clientPhone}) | Entrega:{" "}
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                  {LAUNDRY_STATUS_STEPS.map((step) => (
                    <button
                      key={step.status}
                      onClick={() =>
                        handleUpdateStatus(selectedItem.id, step.status)
                      }
            ...
            <div className="text-center py-28 text-slate-650 text-xs my-auto">
              ...
              Selecciona una prenda en taller para inspeccionar estado de lavado
              y manchas.
            </div>
          )}
        </div>
      ...
      <div className="bg-slate-900/40 p-6 rounded-3xl border border-slate-850 space-y-4">
        <div>
          ...
          <p className="text-[10px] text-slate-500 mt-1">
            Asigná el ticket de prenda seleccionado para iniciar el proceso de
            lavado/secado mecánico y visualizar tiempos restantes en tiempo
            real.
          </p>
        </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              <span className="font-bold text-slate-200 block">
                Control de carga del tambor
              </span>
              <span className="text-[10px] text-slate-400">
                Falta registrar el peso de las prendas y la capacidad de cada
                máquina para calcular carga, agua e insumos.
              </span>
            </div>
          </div>
          <span className="px-2 py-1 bg-cyan-500/10 text-cyan-400 text-[10px] font-bold rounded-lg border border-cyan-500/20 uppercase">
            Pesaje requerido
          </span>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
          {machines.map((m) => {
            ...
            return (
              <div
                key={m.id}
                className="p-4 bg-slate-950/60 rounded-2xl border border-slate-850 space-y-3.5 flex flex-col justify-between text-xs"
              >
                <div>
            ...
            <div className="flex gap-3">
              ...
              <button
                onClick={() => {
                  setFeedback({
                    tone: "success",
                    message: "Ticket / rótulo enviado a impresión.",
                  });
            ...
            <form onSubmit={handleRegisterGarment} className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <select
                    value={newGarment}
                    onChange={(e) => {
                      if (isGarmentType(e.target.value))
                        setNewGarment(e.target.value);
                    }}
                    className="w-full bg-slate-950 border border-slate-800 rounded-xl px-2 py-2 text-xs text-white"
                  >
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <select
                    value={newCycle}
                    onChange={(e) => {
                      if (isWashingCycle(e.target.value))
                ...
                <div>
                  ...
                  <select
                    value={newService}
                    onChange={(e) => {
                      if (isServiceType(e.target.value))
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="col-span-2">
                  <label className="text-[10px] text-slate-500 uppercase font-bold block mb-1">
                    Clasificación Mancha
                  </label>
                  <select
                    value={newStainType}
                    onChange={(e) => {
                      if (isStainType(e.target.value))
                        setNewStainType(e.target.value);
                    }}
                    className="w-full bg-slate-950 border border-slate-800 rounded-xl px-2 py-2 text-xs text-white"
                  >
