...
const INITIAL_PETS: PetProfile[] = [
  {
    ...
    diagnosticNotes:
      "Chequeo general. Roco presenta un leve sobrepeso para su contextura. Controlar alimento.",
  },
  {
    id: "pet-2",
    petName: "Mimi",
...
const INITIAL_GROOMING: GroomingSlot[] = [
  {
    ...
    spotsMax: 4,
    spotsTaken: 4,
    groomer: "Marta Estilista",
    waitingList: ["Coco (Caniche)"],
  },
  {
    id: "g2",
    time: "11:30",
...
export default function PetConsole() {
  ...
  const [groomingSlots, setGroomingSlots] =
    useState<GroomingSlot[]>(INITIAL_GROOMING);
  ...
  const [petShopProducts, setPetShopProducts] = useState([
    {
      id: "prod1",
      name: "Alimento Royal Canin Perro 3kg",
      price: 18500,
      stock: 12,
      category: "Alimento",
    },
    {
      id: "prod2",
      name: "Alimento Pro Plan Gato Urinary 2kg",
      price: 14000,
      stock: 4,
      category: "Alimento",
    },
    {
      id: "prod3",
      name: "Collar Antiparasitario Scalibor",
      price: 9500,
      stock: 20,
      category: "Accesorios",
    },
    {
      id: "prod4",
      name: "Piedras Sanitarias Gato 4kg",
      price: 3500,
      stock: 15,
      category: "Higiene",
    },
  ...
  const [diagNotes, setDiagNotes] = useState(
    INITIAL_PETS[0]?.diagnosticNotes ?? "",
  );
  ...
  const handleCreatePet = (e: React.FormEvent) => {
    ...
    if (!petName || !ownerName || ageMonths <= 0) {
      setFeedback({
        tone: "error",
        message: "Completá mascota, dueño y edad con valores válidos.",
      });
      return;
    }
    ...
    const newPet: PetProfile = {
      ...
      vaccinationHistory: [
        {
          vaccineName: "Antirrábica Obligatoria",
          status: "pending",
          dueDate: todayISO(),
        },
      ],
    ...
    setFeedback({
      tone: "success",
      message: `Ficha clínica creada para ${newPet.petName}.`,
    });
  ...
  const handleLogWeight = (e: React.FormEvent) => {
    ...
    if (!selectedPet || newWeight <= 0) {
      setFeedback({
        tone: "error",
        message: "Ingresá un peso válido mayor a cero.",
      });
      return;
    }
    ...
    setFeedback({
      tone: "success",
      message: `Peso registrado para ${updated.petName}.`,
    });
  };
  ...
  const handleSaveDiagnosis = () => {
    ...
    const updated: PetProfile = {
      ...selectedPet,
      diagnosticNotes: diagNotes.trim(),
    };
    ...
    setFeedback({
      tone: "success",
      message: "Historia clínica y diagnóstico del veterinario guardados.",
    });
  };
  ...
  const handleToggleVaccine = (vaccineIdx: number) => {
    ...
    const vaccinationHistory = selectedPet.vaccinationHistory.map(
      (vaccine, idx) => {
        const nextStatus: VaccineStatus =
          vaccine.status === "applied" ? "pending" : "applied";
        return idx === vaccineIdx
          ? { ...vaccine, status: nextStatus }
          : vaccine;
      },
    );
  ...
  const handleBookGrooming = (slotId: string) => {
    if (!selectedPet) {
      setFeedback({
        tone: "warning",
        message: "Seleccioná una mascota antes de reservar peluquería.",
      });
      return;
    }

    setGroomingSlots((prev) =>
      prev.map((s) => {
        if (s.id === slotId) {
          ...
          if (isFull) {
            setFeedback({
              tone: "warning",
              message: `Peluquería completa: ${selectedPet.petName} fue agregada a lista de espera.`,
            });
            return {
              ...s,
              waitingList: [
                ...s.waitingList,
                `${selectedPet.petName} (Espera)`,
              ],
            };
          } else {
            setFeedback({
              tone: "success",
              message: `Turno de peluquería agendado para ${selectedPet.petName} a las ${s.time} hs.`,
            });
  ...
  const handlePrintRecord = (pet: PetProfile) => {
    const weights = pet.weightHistory
      .map((w) => `- ${w.date}: ${w.weightKg} kg`)
      .join("\n");
    const vaccines = pet.vaccinationHistory
      .map(
        (v) =>
          `- ${v.vaccineName}: ${VACCINE_STATUS_LABELS[v.status]} (${v.dueDate})`,
      )
      .join("\n");
  ...
  const handleSellProduct = (
    ...
    if (success) {
      ...
      setFeedback({
        tone: "success",
        message: `Venta realizada: 1x ${productName} ($${price.toLocaleString()}) registrada en caja.`,
      });
    } else {
      setFeedback({
        tone: "error",
        message: `Sin stock disponible para ${productName}. Por favor reabastecer.`,
      });
    }
  };

  // FASE 3: Reabastecer producto de Pet Shop
  const handleReplenishProduct = (
    productId: string,
    productName: string,
    cost: number,
  ) => {
    if (cashBalance < cost) {
      setFeedback({
        tone: "error",
        message: `Saldo insuficiente en caja para reabastecer ${productName}.`,
      });
      return;
    }
    ...
    setFeedback({
      tone: "success",
      message: `Reabastecimiento completado: +10 unidades de ${productName}. Costo debitado de caja.`,
    });
  };

  // FASE 3: Enviar recordatorio de vacuna por SMS/WhatsApp
  const handleSendVaccineReminder = (
    petName: string,
    vaccineName: string,
    ownerPhone: string,
  ) => {
  ...
  const metrics = useMemo(() => {
    const pendingVaccines = pets.reduce(
      (acc, curr) =>
        acc +
        curr.vaccinationHistory.filter((v) => v.status === "pending").length,
      0,
    );
    const averageVisitRate =
      pets.length > 0
        ? pets.reduce((acc, curr) => acc + curr.visitsFrequencyRate, 0) /
          pets.length
        : 0;

    return {
      activePatients: pets.length,
      groomingTurns: groomingSlots.reduce(
        (acc, curr) => acc + curr.spotsTaken,
        0,
      ),
      pendingVaccines,
      averageVisitRate,
  ...
  return (
    <div className="w-full max-w-7xl mx-auto px-4 py-6 text-slate-100 flex flex-col gap-6 font-sans">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-black text-white">
              Consola de Pacientes Veterinarios & Peluquería
            </h1>
            <p className="text-slate-400 text-xs mt-0.5">
              Fichas clínicas de mascotas, planes de vacunación, recetas
              digitales e historial de pesos.
            </p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
        ...
        <div className="bg-slate-900/50 p-4 rounded-xl border border-slate-850">
          ...
          <span className="text-xl font-black text-indigo-400 mt-1 block">
            {metrics.averageVisitRate.toFixed(1)} /mes
          </span>
        </div>
        <div className="bg-slate-900/50 p-4 rounded-xl border border-slate-850">
          <span className="text-[10px] text-slate-500 font-bold uppercase block">
      ...
      {pets.some((p) =>
        p.vaccinationHistory.some((v) => v.status === "pending"),
      ) && (
        <div className="p-4 bg-amber-550/10 border border-amber-500/25 rounded-2xl flex items-center justify-between">
          <div className="flex items-center gap-2.5 text-xs text-amber-300 font-semibold">
            ...
            <span>
              Alerta Veterinaria: Existen mascotas con vacunas obligatorias
      ...
      <div className="bg-slate-900/40 p-5 rounded-2xl border border-slate-850 grid grid-cols-1 sm:grid-cols-3 gap-3">
        ...
        <div className="space-y-1">
          ...
          <select
            value={speciesFilter}
            onChange={(e) => {
              if (isSpeciesFilter(e.target.value))
                ...
                return (
                  ...
                  >
                    ...
                    <span className="text-[10px] text-slate-400 block mt-1">
                      Dueño: {p.ownerName} | Raza: {p.breed}
                    </span>
                    <span className="text-[10px] text-slate-500 block">
                      Frecuencia: {p.visitsFrequencyRate} visitas/mes
                    </span>
                  </div>
                );
              })
        ...
        <div className="lg:col-span-2 flex flex-col gap-6">
          {selectedPet ? (
            <div className="bg-slate-900/40 p-6 rounded-3xl border border-slate-850 space-y-6">
              <div className="p-3 bg-rose-950/20 border border-rose-500/20 rounded-2xl flex items-center justify-between text-xs">
                <div className="flex items-center gap-2">
                  <Syringe className="h-4 w-4 text-rose-400" />
                  <div>
                    <span className="font-bold text-slate-200 block">
                      Prescripción antiparasitaria
                    </span>
                    <span className="text-[10px] text-slate-400">
                      ...
                      kg. La dosis requiere producto, concentración, especie y
                      validación del veterinario responsable.
                    </span>
                  </div>
                </div>
              ...
              <div className="flex justify-between items-start pb-4 border-b border-slate-850">
                <div>
                  <h4 className="font-black text-base text-white">
                    {selectedPet.petName} ({selectedPet.breed})
                  </h4>
                  <span className="text-[10px] text-slate-500 block mt-0.5">
                    Dueño: {selectedPet.ownerName} ({selectedPet.ownerPhone}) |
                    Alimento: {selectedPet.regularFood}
                  </span>
                </div>
                ...
                <div className="flex gap-2">
                  <button
                    ...
                    aria-label={`Imprimir prescripción de ${selectedPet.petName}`}
              ...
              <div className="p-4 bg-slate-950 border border-slate-850 rounded-2xl space-y-3">
                ...
                <div className="space-y-2">
                  {selectedPet.vaccinationHistory.map((v, idx) => (
                    <div
                      key={idx}
                      className="flex justify-between items-center text-xs p-2.5 bg-slate-900/40 rounded-xl border border-slate-900"
                    >
                      <div className="flex items-center gap-2">
                        <input
                          type="checkbox"
                        ...
                        />
                        <span
                          className={`font-semibold ${v.status === "applied" ? "line-through text-slate-500" : "text-slate-200"}`}
                        >
                          {v.vaccineName}
                        </span>
                      </div>
              ...
              <div className="p-4 bg-slate-950 border border-slate-850 rounded-2xl space-y-4">
                <div className="flex justify-between items-center">
                  <span className="text-[10px] text-slate-555 font-bold uppercase block">
                    Control Balanza Histórica de Peso
                  </span>
                  ...
                  <div className="space-y-1.5 bg-slate-900/30 p-2.5 rounded-xl border border-slate-900">
                    {selectedPet.weightHistory.map((w, i) => {
                      const maxWeight = Math.max(
                        ...selectedPet.weightHistory.map((wh) => wh.weightKg),
                        1,
                      );
                      ...
                      return (
                        ...
                        >
                          <span className="w-16 text-slate-550 font-mono shrink-0">
                            {w.date}
                          </span>
                          ...
                          <span className="w-10 text-right text-slate-350 font-bold font-mono">
                            {w.weightKg} kg
                          </span>
                        </div>
                      );
                ...
                <form onSubmit={handleLogWeight} className="flex gap-2 pt-1">
                  ...
                  <button
                    type="submit"
                    className="px-4 bg-slate-900 hover:bg-slate-850 text-slate-300 border border-slate-800 text-xs font-bold rounded-xl cursor-pointer"
                  >
                    Registrar Peso
                  </button>
                </form>
              ...
              <div className="p-4 bg-slate-950 border border-slate-850 rounded-2xl space-y-3">
                <span className="text-xs font-bold text-rose-450 uppercase block">
                  Diagnóstico de Consulta Médica
                </span>
                <textarea
                  rows={3}
                  value={diagNotes}
                  ...
                  >
                    ...
                    <div className="flex gap-4 items-center w-full md:w-auto border-t md:border-t-0 border-slate-900 pt-3 md:pt-0 justify-between">
                      <div className="text-left md:text-right shrink-0">
                        <span className="text-[9px] text-slate-555 uppercase block font-bold">
                          Cupos
                        </span>
                        <span className="font-black text-rose-450 text-xs">
                          {s.spotsTaken} / {s.spotsMax}
                        </span>
                      </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {petShopProducts.map((p) => {
                ...
                return (
                  <div
                    key={p.id}
                    className="p-3.5 bg-slate-950 border border-slate-850 rounded-2xl flex justify-between items-center gap-4 text-xs"
                  >
                    <div className="space-y-1">
                      <div className="flex items-center gap-2">
                      ...
                      <div className="flex gap-3 text-[10px] text-slate-500">
                        ...
                        <span
                          className={`font-mono ${lowStock ? "text-amber-500 font-bold" : ""}`}
                        >
            ...
            <div className="flex gap-3">
              ...
              <button
                onClick={() => {
                  setFeedback({
                    tone: "success",
                    message: "Documento veterinario enviado a impresión.",
                  });
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <select
                    value={newSpecies}
                    onChange={(e) => {
                      if (isPetSpecies(e.target.value))
                        setNewSpecies(e.target.value);
                    }}
                    className="w-full bg-slate-950 border border-slate-800 rounded-xl px-2 py-2 text-xs text-white"
                  >
                    {SPECIES_FILTERS.filter(
