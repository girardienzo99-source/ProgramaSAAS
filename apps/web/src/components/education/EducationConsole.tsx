...
import {
  GraduationCap, Plus, CheckCircle2, AlertTriangle, BookOpen, Sparkles, Search, Printer, X
} from 'lucide-react';
...
const COURSE_FILTERS: Array<{ label: string; value: CourseFilter }> = [
  ...
  ...COURSES.map(course => ({ label: COURSE_INFO[course].label, value: course }))
];
...
const PAYMENT_STATUS_FILTERS: Array<{ label: string; value: PaymentStatusFilter }> = [
  { label: 'Todos los Estados', value: 'all' },
  ...Object.entries(PAYMENT_STATUS_LABELS).map(([value, label]) => ({ value: value as PaymentStatus, label }))
];
...
const INITIAL_STUDENTS: Student[] = [
  { id: 's1', name: 'Horacio Quiroga', tutorName: 'Self', tutorPhone: '11-5841-2532', course: 'Desarrollo Full Stack React', teacher: 'Prof. Walter', classroom: 'Aula 102 (Lab Informática)', monthlyFee: 45000, paymentStatus: 'paid', attendanceRate: 94, grades: ['9', '10', '9'], observations: 'Excelente desempeño y participación en proyectos.', scholarshipPercent: 0, paymentHistory: [{ date: '2026-07-02', amount: 45000, invoiceNum: 'FAC-ED-00912' }] },
  { id: 's2', name: 'Clara Vignolo', tutorName: 'Carlos Vignolo', tutorPhone: '11-9988-7733', course: 'Diseño UX/UI Avanzado', teacher: 'Dra. María Celeste', classroom: 'Aula 104 (Lab Multimedial)', monthlyFee: 38000, paymentStatus: 'arrears', attendanceRate: 72, grades: ['6', '5'], observations: 'Faltas reiteradas en entregas de portfolios.', scholarshipPercent: 20, paymentHistory: [] },
  { id: 's3', name: 'Marcos Juárez', tutorName: 'Self', tutorPhone: '11-5544-3322', course: 'Desarrollo Full Stack React', teacher: 'Prof. Walter', classroom: 'Aula 102 (Lab Informática)', monthlyFee: 45000, paymentStatus: 'pending', attendanceRate: 85, grades: ['8', '7'], observations: 'Participativo, debe reforzar entregas prácticas.', scholarshipPercent: 0, paymentHistory: [] }
];
...
export default function EducationConsole() {
  ...
  const [newCourse, setNewCourse] = useState<CourseName>('Desarrollo Full Stack React');
  ...
  const [obsText, setObsText] = useState(INITIAL_STUDENTS[0]?.observations ?? '');
  ...
  const handleEnrollStudent = (e: React.FormEvent) => {
    ...
    if (!name || monthlyFee <= 0 || scholarshipPercent < 0 || scholarshipPercent > 100) {
      setFeedback({ tone: 'error', message: 'Completá alumno, cuota y beca con valores válidos.' });
      return;
    }
    ...
    setNewTutor('');
    setNewTutorPhone('');
    setFeedback({ tone: 'success', message: `Alumno ${newStudent.name} matriculado con éxito.` });
  };

  const handleCollectFee = () => {
    if (!selectedStudent) return;

    const finalFee = selectedStudent.monthlyFee * (1 - selectedStudent.scholarshipPercent / 100);
    ...
    const updated: Student = {
      ...
      paymentHistory: [
        ...
        { date: todayISO(), amount: finalFee, invoiceNum: `FAC-ED-${Math.floor(10000 + Math.random() * 90000)}` }
      ]
    };
    ...
    setFeedback({ tone: 'success', message: `Cuota cobrada para ${selectedStudent.name}. Total con beca: $${finalFee.toLocaleString()}.` });
  };
  ...
  const handleAddGrade = (e: React.FormEvent) => {
    ...
    if (!selectedStudent || !grade) {
      setFeedback({ tone: 'error', message: 'Ingresá una calificación válida.' });
      return;
    }
    ...
    setNewGrade('');
    setFeedback({ tone: 'success', message: 'Calificación agregada al boletín.' });
  };

  const handleSaveObservations = () => {
    if (!selectedStudent) return;

    const updated: Student = { ...selectedStudent, observations: obsText.trim() };

    setStudents(prev => prev.map(s => s.id === updated.id ? updated : s));
    setSelectedStudent(updated);

    setFeedback({ tone: 'success', message: 'Observaciones académicas actualizadas.' });
  };
  ...
  const handleGenerateAIReport = () => {
    ...
    const gradesNum = selectedStudent.grades.map(Number).filter(n => !isNaN(n));
    const average = gradesNum.length > 0 
    ...
    setObsText(prev => prev ? `${prev}\n\n[Informe IA ${todayISO()}]: ${suggestion}` : `[Informe IA ${todayISO()}]: ${suggestion}`);
    setFeedback({
      tone: 'success',
  ...
  const handlePrintPaymentReceipt = (student: Student) => {
    const finalFee = student.monthlyFee * (1 - student.scholarshipPercent / 100);
    setPrintModalContent({
  ...
  const metrics = useMemo(() => ({
    ...
    arrearsTotal: students.filter(s => s.paymentStatus === 'arrears').reduce((acc, curr) => acc + curr.monthlyFee, 0),
    averageAttendance: students.length > 0
      ? students.reduce((acc, curr) => acc + curr.attendanceRate, 0) / students.length
      : 0,
  ...
  const filteredStudents = useMemo(() => students.filter(s => {
    const matchSearch = s.name.toLowerCase().includes(searchTerm.toLowerCase()) || s.tutorName.toLowerCase().includes(searchTerm.toLowerCase());
    ...
    <div className="w-full max-w-7xl mx-auto px-4 py-6 text-slate-100 flex flex-col gap-6 font-sans">
      ...
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center bg-slate-900/60 p-5 rounded-2xl border border-slate-800 gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-black text-white">Consola Escolar & Matrícula de Alumnos</h1>
            <p className="text-slate-400 text-xs mt-0.5">Control de cuotas mensuales, boletines de calificaciones y alertas de asistencia.</p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
        <div className="bg-slate-900/50 p-4 rounded-xl border border-slate-850">
          <span className="text-[10px] text-slate-500 font-bold uppercase block">Alumnos Activos</span>
          <span className="text-xl font-black text-white mt-1 block">{metrics.activeStudents}</span>
        </div>
        <div className="bg-slate-900/50 p-4 rounded-xl border border-slate-850">
          <span className="text-[10px] text-slate-500 font-bold uppercase block">Cuotas Cobradas</span>
          <span className="text-xl font-black text-emerald-450 mt-1 block">
            {metrics.paidFees}
          </span>
        </div>
        <div className="bg-slate-900/50 p-4 rounded-xl border border-slate-850">
          <span className="text-[10px] text-slate-500 font-bold uppercase block">Mora Académica</span>
          <span className="text-xl font-black text-rose-500 mt-1 block">${metrics.arrearsTotal.toLocaleString()}</span>
        </div>
        <div className="bg-slate-900/50 p-4 rounded-xl border border-slate-850">
          <span className="text-[10px] text-slate-500 font-bold uppercase block">Asistencia Promedio</span>
          <span className="text-xl font-black text-cyan-400 mt-1 block">
            {metrics.averageAttendance.toFixed(0)}%
          </span>
        </div>
        <div className="bg-slate-900/50 p-4 rounded-xl border border-slate-850">
          <span className="text-[10px] text-slate-500 font-bold uppercase block">Alerta Inasistencias</span>
      ...
      {students.some(s => s.attendanceRate < 75) && (
        <div className="p-4 bg-rose-550/10 border border-rose-500/25 rounded-2xl flex items-center justify-between">
          <div className="flex items-center gap-2.5 text-xs text-rose-350 font-semibold">
            ...
            <span>Alerta Pedagógica: Existen alumnos con inasistencias acumuladas superiores al límite permitido (Libre).</span>
          </div>
        </div>
      )}
      ...
      <div className="bg-slate-900/40 p-5 rounded-2xl border border-slate-850 grid grid-cols-1 sm:grid-cols-3 gap-3">
        <div className="space-y-1">
          <label className="text-[9px] text-slate-500 font-bold uppercase block">Buscar Alumno / Tutor</label>
        ...
        <div className="space-y-1">
          <label className="text-[9px] text-slate-500 font-bold uppercase block">Curso / Comisión</label>
          <select
            value={courseFilter}
            onChange={e => {
              if (isCourseFilter(e.target.value)) setCourseFilter(e.target.value);
            }}
        ...
        <div className="space-y-1">
          <label className="text-[9px] text-slate-500 font-bold uppercase block">Estado Cuota</label>
          <select
            value={statusFilter}
            onChange={e => {
              if (isPaymentStatusFilter(e.target.value)) setStatusFilter(e.target.value);
            }}
            className="w-full bg-slate-950 border border-slate-800 rounded-xl px-2 py-2 text-xs text-white"
          >
              ...
              return (
                <div
                  ...
                  className={`p-4 rounded-2xl text-left border cursor-pointer transition-all ${
                    active ? 'bg-violet-950/10 border-violet-500/40 text-white' : 'bg-slate-950/40 border-slate-850 hover:border-slate-800'
                  }`}
                >
                  <div className="flex justify-between items-center w-full">
                    <span className="font-extrabold text-xs text-white">{s.name}</span>
                    {alarm && <span className="text-[8px] bg-rose-500/10 text-rose-450 px-1.5 py-0.5 rounded font-bold uppercase">Libre/Faltas</span>}
                  </div>
                  <span className="text-[10px] text-slate-400 block mt-1">{s.course}</span>

                  <div className="mt-3 flex justify-between items-center border-t border-slate-900 pt-2 text-[10px]">
                    <span className={`px-2 py-0.5 rounded text-[8px] font-bold uppercase ${
                      s.paymentStatus === 'paid' ? 'bg-emerald-500/10 text-emerald-450 border border-emerald-500/20' :
                      s.paymentStatus === 'arrears' ? 'bg-rose-500/10 text-rose-400 border border-rose-500/20' :
                      'bg-amber-500/10 text-amber-400 border border-amber-500/20'
                    }`}>
                      {PAYMENT_STATUS_LABELS[s.paymentStatus]}
                    </span>
                    <span className="text-slate-500">Asistencia: {s.attendanceRate}%</span>
                  </div>
                </div>
              );
            })}
          ...
          {selectedStudent ? (
            <div className="bg-slate-900/40 p-6 rounded-3xl border border-slate-850 space-y-6">
              ...
              <div className="flex justify-between items-start pb-4 border-b border-slate-850">
                <div>
                  ...
                  <span className="text-[10px] text-slate-500 block mt-0.5">
                    Curso: {selectedStudent.course} | Tutor: {selectedStudent.tutorName} (${selectedStudent.tutorPhone})
                  </span>
                </div>
              ...
              <div className="p-4 bg-slate-950 border border-slate-850 rounded-2xl text-xs space-y-2">
                <span className="text-[10px] text-slate-550 font-bold uppercase block">Datos de Matriculación e Infraestructura</span>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  <div>
                    <span>Docente a Cargo</span>
                    <span className="font-bold text-slate-300 block mt-0.5">{selectedStudent.teacher}</span>
                  </div>
                  <div>
                    <span>Aula Asignada</span>
                    <span className="font-bold text-slate-300 block mt-0.5">{selectedStudent.classroom}</span>
                  </div>
                  <div>
                    <span>Descuento Beca</span>
                    <span className="font-bold text-slate-300 block mt-0.5">{selectedStudent.scholarshipPercent}%</span>
                  </div>
                  <div>
                    <span>Asistencia</span>
                    <span className={`font-bold block mt-0.5 ${selectedStudent.attendanceRate < 75 ? 'text-rose-500' : 'text-emerald-450'}`}>
              ...
              <div className="p-4 bg-slate-950 border border-slate-850 rounded-2xl flex justify-between items-center text-xs">
                <div className="space-y-0.5 text-left">
                  <span className="text-[10px] text-slate-550 font-bold uppercase block">Asistencia Diaria (Clase de Hoy)</span>
                  <span className="text-slate-500">Registrar presentismo para recalcular el porcentaje académico.</span>
                </div>
              ...
              <div className="p-4 bg-slate-950 border border-slate-850 rounded-2xl space-y-3">
                ...
                <div className="flex gap-2 items-center flex-wrap pt-1 border-b border-slate-900 pb-3">
                  {selectedStudent.grades.length === 0 ? (
                    <span className="text-[10px] text-slate-500 italic">No hay calificaciones cargadas aún.</span>
                  ) : (
                    selectedStudent.grades.map((grade, idx) => (
                      <span key={idx} className="bg-slate-900 border border-slate-800 text-white font-bold text-xs px-3 py-1 rounded-xl">
                        Nota: {grade}
                      </span>
                    ))
                  )}
                ...
                <form onSubmit={handleAddGrade} className="flex gap-2 pt-1">
                  ...
                  />
                  <button type="submit" className="px-4 bg-slate-900 border border-slate-800 text-slate-350 hover:bg-slate-850 rounded-xl text-xs">
              ...
              <div className="p-4 bg-slate-950 border border-slate-850 rounded-2xl space-y-3">
                <span className="text-xs font-bold text-slate-400 uppercase block">Observaciones del Pedagogo</span>
                <textarea
                  rows={2}
                  value={obsText}
        ...
        <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white border border-slate-200 rounded-3xl p-6 max-w-md w-full space-y-6 shadow-2xl text-slate-900 animate-fade-in">
            ...
            <div className="flex gap-3">
              ...
              <button
                onClick={() => {
                  setFeedback({ tone: 'success', message: 'Comprobante educativo enviado a impresión.' });
      ...
      {isAddModalOpen && (
        <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 w-full max-w-md shadow-2xl">
            <h2 className="text-xl font-black mb-4 text-white">Matricular Nuevo Alumno</h2>
            <form onSubmit={handleEnrollStudent} className="space-y-4">
              <div>
                <label className="text-[10px] text-slate-500 uppercase font-bold block mb-1">Nombre Completo</label>
                <input
                  type="text"
                  required
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="text-[10px] text-slate-500 uppercase font-bold block mb-1">Tutor / Responsable</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="text-[10px] text-slate-500 uppercase font-bold block mb-1">Celular Tutor</label>
                  <input
                    type="text"
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="text-[10px] text-slate-500 uppercase font-bold block mb-1">Beca Otorgada (%)</label>
                  <input
                    type="number"
                ...
                <div>
                  <label className="text-[10px] text-slate-500 uppercase font-bold block mb-1">Valor Cuota ($)</label>
                  <input
                    type="number"
                    required
                    value={newFee}
              ...
              <div>
                <label className="text-[10px] text-slate-500 uppercase font-bold block mb-1">Curso / Comisión</label>
                <select
                  value={newCourse}
                  onChange={e => {
                    if (isCourseName(e.target.value)) setNewCourse(e.target.value);
                  }}
                  className="w-full bg-slate-950 border border-slate-800 rounded-xl px-2 py-2 text-xs text-white"
                >
                  {COURSES.map(course => (
                    <option key={course} value={course}>{COURSE_INFO[course].label}</option>
                  ))}
                </select>
              </div>
